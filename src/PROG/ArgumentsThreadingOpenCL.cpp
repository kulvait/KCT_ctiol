#include "PROG/ArgumentsThreadingOpenCL.hpp"

using namespace KCT;
using namespace KCT::util;

void ArgumentsThreadingOpenCL::addThreadingArgs(CLI::Option_group* og) 
{
    ...
    // If strictThreadRange is set, cap the max thread count to the number of OpenCL GPUs.
    ...
    help = io::xprintf(
        "Number of threads that the program can use, default is the number of OpenCL devices %d. "
        "Value must be in the range [%d, %d]. Negative values represent "
        "fractions of OpenCL devices (e.g., -1 means use %d devices, -2 means use %d devices).",
    ...
    auto thread_callback = [this, inputRangeMax](const int32_t& value) {
        if(value < 0)
        {
            // Negative value means fraction of OpenCL devices
