...
inline std::size_t
for_each_token(const std::string& buffer, Tokenizer& tokenizer, Function function)
{
...
template <typename T>
inline bool
read_line_as_value(std::istream& stream, T& t, const std::size_t& buffer_size = one_kilobyte)
{
    std::string buffer;
    buffer.reserve(buffer_size);
...
namespace details {
    ...
    {
        typedef
            typename details::enable_if<details::supported_iterator_type<T>::value, T>::type type;
    };
    ...
    register_stl_container1(std::vector) register_stl_container1(std::deque)
        register_stl_container1(std::list) register_stl_container1(std::queue)
            register_stl_container1(std::stack) register_stl_container2(std::set)
                register_stl_container2(std::multiset) register_stl_container2(std::priority_queue)
...
inline const std::string::value_type* to_ptr(const std::string& s) { return &s[0]; }
...
#define strtk_string_to_type_begin(Type)                                                           \
    namespace strtk {                                                                              \
        namespace details {                                                                        \
            ...
            inline bool string_to_type_converter_impl(const Iterator& begin,                       \
                                                      const Iterator& end,                         \
                                                      Type& t,                                     \
...
inline std::size_t
load_from_text_file(std::istream& stream,
...
template <typename T, typename Container, typename Comparator>
inline std::size_t
load_from_text_file(const std::string& file_name,
                    std::priority_queue<T, Container, Comparator>& priority_queue,
                    const std::size_t& buffer_size = one_kilobyte)
{
    std::ifstream stream(file_name.c_str());
...
inline void
copy_if(Predicate predicate, const InputIterator begin, const InputIterator end, OutputIterator out)
{
...
template <typename InputIterator, typename OutputIterator>
inline void extract_unique(const InputIterator begin, const InputIterator end, OutputIterator out)
{
...
inline bool
range_only_contains(Predicate predicate, const InputIterator begin, const InputIterator end)
{
    InputIterator itr = begin;
...
namespace range {
    ...
    public:
        ...
        inline const T& operator[](const std::size_t& index) const { return *(begin_ + index); }
    ...
    template <typename T, typename Allocator, template <typename, typename> class Sequence>
    inline adapter<typename Sequence<T, Allocator>::iterator>
    type(const Sequence<T, Allocator>& seq)
    {
...
public:
    ...
    inline bool operator()(const unsigned char& c) const { return (delimiter_table_[c]); }
...
template <typename Allocator, template <typename, typename> class Sequence>
inline details::index_remover_impl<Allocator, Sequence>
index_remover(const Sequence<std::size_t, Allocator>& sequence)
{
    return details::index_remover_impl<Allocator, Sequence>(sequence);
}

template <typename Iterator, typename Predicate>
inline std::size_t remove_inplace(Predicate predicate, Iterator begin, Iterator end)
{
    Iterator itr1 = begin;
    Iterator itr2 = begin;
...
inline void remove_inplace(const std::string::value_type c, std::string& s)
{
    const std::size_t removal_count
        = remove_inplace(single_delimiter_predicate<std::string::value_type>(c),
...
{
    const std::size_t removal_count = remove_inplace(predicate, const_cast<char*>(to_ptr(s)),
...
template <typename Iterator, typename Predicate>
inline std::size_t remove_consecutives_inplace(Predicate predicate, Iterator begin, Iterator end)
{
...
inline void remove_consecutives_inplace(const std::string::value_type c, std::string& s)
{
    ...
    const std::size_t removal_count = remove_consecutives_inplace(
        single_delimiter_predicate<std::string::value_type>(c), const_cast<char*>(to_ptr(s)),
        const_cast<char*>(to_ptr(s) + s.size()));
    if(removal_count > 0)
    {
        s.resize(s.size() - removal_count);
...
inline void remove_consecutives_inplace(const std::string& rem_chars, std::string& s)
{
    if(s.empty())
        return;
    const std::size_t removal_count = remove_consecutives_inplace(
        multiple_char_delimiter_predicate(rem_chars), const_cast<char*>(to_ptr(s)),
        const_cast<char*>(to_ptr(s) + s.size()));
    if(removal_count > 0)
    {
        s.resize(s.size() - removal_count);
...
namespace details {
#if(defined(__MINGW32_VERSION)) || (defined(__CYGWIN__) || defined(__CYGWIN32__))                  \
...
inline void remove_consecutives_inplace(Predicate predicate, std::string& s)
{
    if(s.empty())
        return;
    const std::size_t removal_count = remove_consecutives_inplace(
        predicate, const_cast<char*>(to_ptr(s)), const_cast<char*>(to_ptr(s) + s.size()));
    if(removal_count > 0)
    {
        s.resize(s.size() - removal_count);
    }
...
inline std::string remove_duplicates_inplace(std::string& str) { return remove_duplicates(str); }

template <typename Iterator, typename Predicate>
inline std::size_t remove_trailing(Predicate predicate, Iterator begin, Iterator end)
{
...
inline void remove_trailing(const std::string::value_type c, std::string& s)
{
    if(s.empty())
        return;
    const std::size_t removal_count
        = remove_trailing(single_delimiter_predicate<std::string::value_type>(c),
                          const_cast<char*>(to_ptr(s)), const_cast<char*>(to_ptr(s) + s.size()));
    if(removal_count > 0)
    {
        s.resize(s.size() - removal_count);
...
inline void remove_trailing(const std::string& rem_chars, std::string& s)
{
    if(s.empty())
        return;
    const std::size_t removal_count
        = remove_trailing(multiple_char_delimiter_predicate(rem_chars),
                          const_cast<char*>(to_ptr(s)), const_cast<char*>(to_ptr(s) + s.size()));
    if(removal_count > 0)
    {
        s.resize(s.size() - removal_count);
...
{
    const std::size_t removal_count
        = remove_trailing(multiple_char_delimiter_predicate(
...
inline void remove_trailing(Predicate predicate, std::string& s)
{
    if(s.empty())
        return;
    const std::size_t removal_count = remove_trailing(predicate, const_cast<char*>(to_ptr(s)),
                                                      const_cast<char*>(to_ptr(s) + s.size()));
    if(removal_count > 0)
    {
        s.resize(s.size() - removal_count);
...
template <typename Iterator, typename Predicate>
inline std::size_t remove_leading(Predicate predicate, Iterator begin, Iterator end)
{
    const std::size_t length = std::distance(begin, end);
    if(0 == length)
        return 0;
...
inline void remove_leading(const std::string::value_type c, std::string& s)
{
    if(s.empty())
        return;
    const std::size_t removal_count
        = remove_leading(single_delimiter_predicate<std::string::value_type>(c),
                         const_cast<char*>(to_ptr(s)), const_cast<char*>(to_ptr(s) + s.size()));
    if(removal_count > 0)
    {
        s.resize(s.size() - removal_count);
...
inline void remove_leading(const std::string& rem_chars, std::string& s)
{
    if(s.empty())
        return;
    const std::size_t removal_count
        = remove_leading(multiple_char_delimiter_predicate(rem_chars), const_cast<char*>(to_ptr(s)),
                         const_cast<char*>(to_ptr(s) + s.size()));
    if(removal_count > 0)
    {
        s.resize(s.size() - removal_count);
...
inline void remove_leading(const char* rem_chars, std::string& s)
{
    if(s.empty())
        return;
    const std::size_t removal_count
        = remove_leading(multiple_char_delimiter_predicate(
                             rem_chars, rem_chars + details::strnlength(rem_chars, 256)),
                         const_cast<char*>(to_ptr(s)), const_cast<char*>(to_ptr(s) + s.size()));
    if(removal_count > 0)
    {
...
inline void remove_leading(Predicate predicate, std::string& s)
{
    if(s.empty())
        return;
    const std::size_t removal_count = remove_leading(predicate, const_cast<char*>(to_ptr(s)),
                                                     const_cast<char*>(to_ptr(s) + s.size()));
    if(removal_count > 0)
    {
        s.resize(s.size() - removal_count);
...
inline void
replace(const std::string::value_type& c0, const std::string::value_type& c1, std::string& s)
{
    replace(c0, c1, const_cast<char*>(to_ptr(s)), const_cast<char*>(to_ptr(s) + s.size()));
}
...
inline void remove_pattern(const std::string& s, const std::string& p, std::string& n)
{
...
namespace details {
    ...
    inline bool
    match_impl(const Iterator pattern_begin,
        ...
        while((p_itr != pattern_end) && (d_itr != data_end))
        {
            if(match_one_or_more == *p_itr)
            {
                while((p_itr != pattern_end)
                      && (*p_itr == match_one_or_more || *p_itr == match_exactly_one))
                {
                    ++p_itr;
                }
...
inline bool match(const Iterator pattern_begin,
                  ...
                  const typename std::iterator_traits<Iterator>::value_type& match_exactly_one)
{
    return details::match_impl<details::cs_match>(pattern_begin, pattern_end, data_begin, data_end,
                                                  match_one_or_more, match_exactly_one);
}
...
inline bool match(const std::string& wild_card, const std::string& str)
{
    ...
    return details::match_impl<details::cs_match>(to_ptr(wild_card),
                                                  to_ptr(wild_card) + wild_card.size(), to_ptr(str),
                                                  to_ptr(str) + str.size(), '*', '?');
}
...
inline bool imatch(const InputIterator begin1,
...
{
    return imatch(to_ptr(s1), to_ptr(s1) + s1.size(), to_ptr(s2), to_ptr(s2) + s2.size());
}
...
inline std::size_t ifind(const std::string& pattern, const std::string& data)
{
    ...
    const char* result_itr = std::search(to_ptr(data), to_ptr(data) + data.size(), to_ptr(pattern),
...
inline std::size_t find_all(const std::string& pattern, const std::string& data, OutputIterator out)
{
    return find_all(to_ptr(pattern), to_ptr(pattern) + pattern.size(), to_ptr(data),
                    to_ptr(data) + data.size(), out);
}

template <typename Range, typename Allocator, template <typename, typename> class Sequence>
inline std::size_t
find_all(const std::string& pattern, const std::string& data, Sequence<Range, Allocator>& seq)
{
...
template <typename OutputIterator>
inline std::size_t
ifind_all(const std::string& pattern, const std::string& data, OutputIterator out)
{
    return ifind_all(to_ptr(pattern), to_ptr(pattern) + pattern.size(), to_ptr(data),
                     to_ptr(data) + data.size(), out);
}

template <typename Range, typename Allocator, template <typename, typename> class Sequence>
inline std::size_t
ifind_all(const std::string& pattern, const std::string& data, Sequence<Range, Allocator>& seq)
{
...
inline bool begins_with(const std::string& pattern, const std::string& data)
{
    if(pattern.size() <= data.size())
    {
        return begins_with(to_ptr(pattern), to_ptr(pattern) + pattern.size(), to_ptr(data),
...
inline bool ibegins_with(const std::string& pattern, const std::string& data)
{
    if(pattern.size() <= data.size())
    {
        return ibegins_with(to_ptr(pattern), to_ptr(pattern) + pattern.size(), to_ptr(data),
                            to_ptr(data) + data.size());
    } else
        return false;
}
...
inline bool ends_with(const std::string& pattern, const std::string& data)
{
    if(pattern.size() <= data.size())
    {
        return ends_with(to_ptr(pattern), to_ptr(pattern) + pattern.size(), to_ptr(data),
                         to_ptr(data) + data.size());
    } else
        return false;
}
...
inline bool iends_with(const std::string& pattern, const std::string& data)
{
    if(pattern.size() <= data.size())
    {
        return iends_with(to_ptr(pattern), to_ptr(pattern) + pattern.size(), to_ptr(data),
                          to_ptr(data) + data.size());
    } else
        return false;
}
...
inline std::size_t index_of(const std::string& pattern, const std::string& data)
{
    ...
    const char* itr = std::search(to_ptr(data), to_ptr(data) + data.size(), to_ptr(pattern),
    ...
    public:
        explicit inline tokenizer_iterator(const iterator begin,
            ...
            , include_all_delimiters_(
                  tokenize_options::perform_include_all_delimiters(tokenize_option))
        ...
        inline std::string remaining() const { return std::string(current_token_.first, end_); }
...
public:
    ...
    inline void assign(const std::string& s) const { assign(to_ptr(s), to_ptr(s) + s.size()); }

    inline void assign(const std::string& s) { assign(to_ptr(s), to_ptr(s) + s.size()); }
...
public:
    ...
    inline range_to_type_back_inserter_iterator&
    operator=(const range_to_type_back_inserter_iterator& itr)
    {
        if(this != &itr)
        {
    ...
    inline range_to_type_back_inserter_iterator operator++(int) { return (*this); }
...
public:
    ...
    inline range_to_type_push_inserter_iterator&
    operator=(const range_to_type_push_inserter_iterator& itr)
    {
        if(this != &itr)
        {
    ...
    inline range_to_type_push_inserter_iterator operator++(int) { return (*this); }
...
class back_inserter_with_valuetype_iterator
    : public std::
          iterator<std::output_iterator_tag, typename Sequence::value_type, void, void, void>
{
public:
    ...
    inline back_inserter_with_valuetype_iterator&
    operator=(const back_inserter_with_valuetype_iterator& itr)
    {
        if(this != &itr)
        {
            this->sequence_ = itr.sequence_;
    ...
    inline void operator()(const typename Sequence::value_type& v) { sequence_.push_back(v); }
...
class inserter_with_valuetype_iterator
    : public std::iterator<std::output_iterator_tag, typename Set::value_type, void, void, void>
{
public:
                         ...
                         const split_options::type split_option = split_options::default_mode)
{
    ...
    const bool include_all_delimiters
        = (!include_1st_delimiter) && split_options::perform_include_all_delimiters(split_option);
                         ...
                         const split_options::type split_option = split_options::default_mode)
{
    if(1 == details::strnlength(delimiters, 256))
        ...
        return split(multiple_char_delimiter_predicate(delimiters), range.first, range.second, out,
                     split_option);
}
...
inline std::size_t split(const std::string::value_type delimiter,
                         ...
                         const split_options::type& split_option = split_options::default_mode)
{
    return split(single_delimiter_predicate<std::string::value_type>(delimiter), to_ptr(str),
                 to_ptr(str) + str.size(), out, split_option);
}
                         ...
                         const split_options::type& split_option = split_options::default_mode)
{
    if(1 == details::strnlength(delimiters, 256))
        ...
        return split(multiple_char_delimiter_predicate(delimiters), to_ptr(str),
                     to_ptr(str) + str.size(), std::back_inserter(sequence), split_option);
}
...
inline std::size_t split(const DelimiterPredicate& delimiter,
                         ...
                         const split_options::type& split_option = split_options::default_mode)
{
    return split(delimiter, to_ptr(str), to_ptr(str) + str.size(), out, split_option);
}
                           ...
                           const split_options::type& split_option = split_options::default_mode)
{
    ...
    details::convert_type_assert<itr_type>();
    if(1 == delimiters.size())
        return split_n(single_delimiter_predicate<std::string::value_type>(delimiters[0]), begin,
                       end, token_count, out, split_option);
    else
        return split_n(multiple_char_delimiter_predicate(delimiters), begin, end, token_count, out,
                       split_option);
}
...
inline std::size_t split_n(const std::string::value_type delimiter,
                           ...
                           const split_options::type& split_option = split_options::default_mode)
{
    return split_n(single_delimiter_predicate<std::string::value_type>(delimiter), to_ptr(str),
                   to_ptr(str) + str.size(), token_count, out, split_option);
}
...
inline std::size_t split_n(const DelimiterPredicate& delimiter,
                           ...
                           const split_options::type& split_option = split_options::default_mode)
{
    return split_n(delimiter, to_ptr(str), to_ptr(str) + str.size(), token_count, out,
                   split_option);
}
...
static const std::string
    uri_expression("((https?|ftp)\\://"
                   "((\\[?(\\d{1,3}\\.){3}\\d{1,3}\\]?)|(([-a-zA-Z0-9]+\\.)+[a-zA-Z]{2,4}))(\\:\\d+"
                   ")?(/[-a-zA-Z0-9._?,+&amp;%$#=~\\\\]+)*/?)");
...
inline std::size_t split_regex_n(const std::string& delimiter_expression,
                                 ...
                                 const regex_match_mode::type mode = regex_match_mode::match_all)
{
    return split_regex_n(delimiter_expression, text.begin(), text.end(), token_count, out, mode);
}
...
inline offset_predicate<12> offsets(const int& v1,
                                    const int& v2,
                                    const int& v3,
                                    const int& v4,
                                    const int& v5,
...
inline offset_predicate<11> offsets(const int& v1,
                                    const int& v2,
                                    const int& v3,
                                    const int& v4,
                                    const int& v5,
...
inline offset_predicate<10> offsets(const int& v1,
                                    const int& v2,
                                    const int& v3,
                                    const int& v4,
                                    const int& v5,
...
inline offset_predicate<9> offsets(const int& v1,
                                   const int& v2,
                                   const int& v3,
                                   const int& v4,
                                   const int& v5,
...
inline offset_predicate<8> offsets(const int& v1,
                                   const int& v2,
                                   const int& v3,
                                   const int& v4,
                                   const int& v5,
...
inline offset_predicate<7> offsets(const int& v1,
                                   const int& v2,
                                   const int& v3,
                                   const int& v4,
                                   const int& v5,
...
inline offset_predicate<6> offsets(const int& v1,
                                   const int& v2,
                                   const int& v3,
                                   const int& v4,
                                   const int& v5,
...
inline offset_predicate<5> offsets(const int& v1,
                                   const int& v2,
                                   const int& v3,
                                   const int& v4,
                                   const int& v5,
...
inline offset_predicate<4>
offsets(const int& v1, const int& v2, const int& v3, const int& v4, const bool& rotate = false)
{
    const int offset_list[4] = { v1, v2, v3, v4 };
    return offset_predicate<4>(offset_list, rotate);
}

inline offset_predicate<3>
offsets(const int& v1, const int& v2, const int& v3, const bool& rotate = false)
{
...
template <typename OffsetPredicate, typename InputIterator, typename OutputIterator>
...
inline std::size_t
offset_splitter(const std::string& str, const OffsetPredicate& offset, OutputIterator out)
{
...
template <typename Function>
inline std::size_t
for_each_token(const std::string& buffer, const std::string& delimiters, Function function)
{
    return split(delimiters, buffer, strtk::functional_inserter<Function>(function));
}

template <typename Function>
inline std::size_t
for_each_token(const std::string& buffer, const char* delimiters, Function function)
{
    return split(delimiters, buffer, strtk::functional_inserter<Function>(function));
}
...
template <typename T, typename Allocator, template <typename, typename> class Sequence>
inline void min_max_of_cont(const Sequence<T, Allocator>& sequence, T& min_value, T& max_value)
{
...
template <typename T, typename Comparator, typename Allocator>
inline void
min_max_of_cont(const std::set<T, Comparator, Allocator>& set, T& min_value, T& max_value)
{
    min_value = (*set.begin());
...
template <typename T, typename Comparator, typename Allocator>
inline void
min_max_of_cont(const std::multiset<T, Comparator, Allocator>& multiset, T& min_value, T& max_value)
{
                                                    ...
                                                    const unsigned char* end)
{
    ...
    return const_cast<const unsigned char*>(
        reinterpret_cast<unsigned char*>(const_cast<char*>(first_non_repeated_char(b, e))));
}
...
inline void
convert_bin_to_hex(const unsigned char* begin, const unsigned char* end, unsigned char* out)
{
    static const unsigned short hex_lut[]
        = { 0x3030, 0x3130, 0x3230, 0x3330, 0x3430, 0x3530, 0x3630, 0x3730, 0x3830, 0x3930, 0x4130,
            0x4230, 0x4330, 0x4430, 0x4530, 0x4630, 0x3031, 0x3131, 0x3231, 0x3331, 0x3431, 0x3531,
            0x3631, 0x3731, 0x3831, 0x3931, 0x4131, 0x4231, 0x4331, 0x4431, 0x4531, 0x4631, 0x3032,
            0x3132, 0x3232, 0x3332, 0x3432, 0x3532, 0x3632, 0x3732, 0x3832, 0x3932, 0x4132, 0x4232,
            0x4332, 0x4432, 0x4532, 0x4632, 0x3033, 0x3133, 0x3233, 0x3333, 0x3433, 0x3533, 0x3633,
            0x3733, 0x3833, 0x3933, 0x4133, 0x4233, 0x4333, 0x4433, 0x4533, 0x4633, 0x3034, 0x3134,
            0x3234, 0x3334, 0x3434, 0x3534, 0x3634, 0x3734, 0x3834, 0x3934, 0x4134, 0x4234, 0x4334,
            0x4434, 0x4534, 0x4634, 0x3035, 0x3135, 0x3235, 0x3335, 0x3435, 0x3535, 0x3635, 0x3735,
            0x3835, 0x3935, 0x4135, 0x4235, 0x4335, 0x4435, 0x4535, 0x4635, 0x3036, 0x3136, 0x3236,
            0x3336, 0x3436, 0x3536, 0x3636, 0x3736, 0x3836, 0x3936, 0x4136, 0x4236, 0x4336, 0x4436,
            0x4536, 0x4636, 0x3037, 0x3137, 0x3237, 0x3337, 0x3437, 0x3537, 0x3637, 0x3737, 0x3837,
            0x3937, 0x4137, 0x4237, 0x4337, 0x4437, 0x4537, 0x4637, 0x3038, 0x3138, 0x3238, 0x3338,
            0x3438, 0x3538, 0x3638, 0x3738, 0x3838, 0x3938, 0x4138, 0x4238, 0x4338, 0x4438, 0x4538,
            0x4638, 0x3039, 0x3139, 0x3239, 0x3339, 0x3439, 0x3539, 0x3639, 0x3739, 0x3839, 0x3939,
            0x4139, 0x4239, 0x4339, 0x4439, 0x4539, 0x4639, 0x3041, 0x3141, 0x3241, 0x3341, 0x3441,
            0x3541, 0x3641, 0x3741, 0x3841, 0x3941, 0x4141, 0x4241, 0x4341, 0x4441, 0x4541, 0x4641,
            0x3042, 0x3142, 0x3242, 0x3342, 0x3442, 0x3542, 0x3642, 0x3742, 0x3842, 0x3942, 0x4142,
            0x4242, 0x4342, 0x4442, 0x4542, 0x4642, 0x3043, 0x3143, 0x3243, 0x3343, 0x3443, 0x3543,
            0x3643, 0x3743, 0x3843, 0x3943, 0x4143, 0x4243, 0x4343, 0x4443, 0x4543, 0x4643, 0x3044,
            0x3144, 0x3244, 0x3344, 0x3444, 0x3544, 0x3644, 0x3744, 0x3844, 0x3944, 0x4144, 0x4244,
            0x4344, 0x4444, 0x4544, 0x4644, 0x3045, 0x3145, 0x3245, 0x3345, 0x3445, 0x3545, 0x3645,
            0x3745, 0x3845, 0x3945, 0x4145, 0x4245, 0x4345, 0x4445, 0x4545, 0x4645, 0x3046, 0x3146,
            0x3246, 0x3346, 0x3446, 0x3546, 0x3646, 0x3746, 0x3846, 0x3946, 0x4146, 0x4246, 0x4346,
            0x4446, 0x4546, 0x4646 };
...
inline void convert_bin_to_hex(const std::pair<unsigned char*, unsigned char*>& r,
                               unsigned char* out)
{
    convert_bin_to_hex(r.first, r.second, out);
}

inline void convert_bin_to_hex(const std::pair<const unsigned char*, const unsigned char*>& r,
                               unsigned char* out)
{
    convert_bin_to_hex(r.first, r.second, out);
}
...
inline void convert_bin_to_hex(const std::string& binary_data, std::string& output)
{
    ...
    convert_bin_to_hex(to_ptr(binary_data), to_ptr(binary_data) + binary_data.size(),
...
inline bool
convert_hex_to_bin(const unsigned char* begin, const unsigned char* end, unsigned char* out)
{
    const std::size_t length = std::distance(begin, end);
    ...
    {
        *reinterpret_cast<unsigned char*>(out)
            = static_cast<unsigned char>(hex_to_bin[itr[0]] << 4 | hex_to_bin[itr[1]]);
        ++out;
        itr += 2;
    }
...
inline bool convert_hex_to_bin(const std::pair<unsigned char*, unsigned char*>& r,
                               unsigned char* out)
{
    return convert_hex_to_bin(r.first, r.second, out);
}

inline bool convert_hex_to_bin(const std::pair<const unsigned char*, const unsigned char*>& r,
                               unsigned char* out)
{
    return convert_hex_to_bin(r.first, r.second, out);
}
...
inline bool convert_hex_to_bin(const std::string& hex_data, std::string& output)
{
    ...
    return convert_hex_to_bin(to_ptr(hex_data), to_ptr(hex_data) + hex_data.size(),
                              const_cast<char*>(to_ptr(output)));
}

inline std::size_t
convert_bin_to_base64(const unsigned char* begin, const unsigned char* end, unsigned char* out)
{
...
inline void convert_bin_to_base64(const std::string& binary_data, std::string& output)
{
    ...
    std::size_t resize
        = convert_bin_to_base64(to_ptr(binary_data), to_ptr(binary_data) + binary_data.size(),
                                const_cast<char*>(to_ptr(output)));
    output.resize(resize);
}

inline std::size_t
convert_base64_to_bin(const unsigned char* begin, const unsigned char* end, unsigned char* out)
{
...
inline void convert_base64_to_bin(const std::string& binary_data, std::string& output)
{
    ...
    std::size_t resize
        = convert_base64_to_bin(to_ptr(binary_data), to_ptr(binary_data) + binary_data.size(),
                                const_cast<char*>(to_ptr(output)));
    output.resize(resize);
}
...
inline bool twoway_bitwise_interleave(
    const char* begin1, const char* end1, const char* begin2, const char* end2, char* out)
{
...
inline bool
twoway_bitwise_interleave(const std::string& str1, const std::string& str2, std::string& out)
{
    ...
    return twoway_bitwise_interleave(to_ptr(str1), to_ptr(str1) + str1.size(), to_ptr(str2),
                                     to_ptr(str2) + str2.size(), const_cast<char*>(to_ptr(out)));
}
                              ...
                              std::string& out)
{
    if(str1.size() != str2.size())
        return;
    out.resize(str1.size());
    bitwise_transform(operation, to_ptr(str1), to_ptr(str1) + str1.size(), to_ptr(str2),
...
inline std::size_t high_bit_count(const unsigned int& i)
{
    ...
    return high_bit_count(*(ptr + 0)) + high_bit_count(*(ptr + 1)) + high_bit_count(*(ptr + 2))
        + high_bit_count(*(ptr + 3));
}
...
inline std::size_t high_bit_count(const long long int& ll)
{
    ...
    return high_bit_count(*(ptr + 0)) + high_bit_count(*(ptr + 1)) + high_bit_count(*(ptr + 2))
        + high_bit_count(*(ptr + 3)) + high_bit_count(*(ptr + 4)) + high_bit_count(*(ptr + 5))
...
inline std::size_t
hamming_distance(const char* begin1, const char* end1, const char* begin2, const char* end2)
{
...
{
    return hamming_distance(to_ptr(str1), to_ptr(str1) + str1.size(), to_ptr(str2),
                            to_ptr(str2) + str2.size());
}
...
{
    return hamming_distance_elementwise(to_ptr(str1), to_ptr(str1) + str1.size(), to_ptr(str2),
                                        to_ptr(str2) + str2.size());
}
...
private:
    ...
    struct row_processor
    {
        ...
        inline void operator()(const range_t& range)
        {
            ...
            std::size_t token_count = split(token_predicate_, range.first, range.second,
...
public:
    ...
    public:
        ...
        inline range_t token(const std::size_t& index) const { return *(begin_ + index); }
        ...
        inline bool
        parse_with_index(const std::size_t& col0, const std::size_t& col1, T0& t0, T1& t1) const
        {
            if(!process(*(begin_ + col0), t0))
                return false;
            if(!process(*(begin_ + col1), t1))
        ...
        template <typename T0,
                  ...
                  typename T9>
        inline bool
        parse(T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5, T6& t6, T7& t7, T8& t8, T9& t9) const
        {
        ...
        template <typename T0,
                  ...
                  typename T8>
        inline bool
        parse(T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5, T6& t6, T7& t7, T8& t8) const
        {
            if(!process(*(begin_ + 0), t0))
                return false;
            if(!process(*(begin_ + 1), t1))
        ...
        template <typename T0,
                  ...
                  typename T6,
                  typename T7>
        inline bool parse(T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5, T6& t6, T7& t7) const
        {
            if(!process(*(begin_ + 0), t0))
                return false;
            if(!process(*(begin_ + 1), t1))
        ...
        template <typename T0,
                  ...
                  typename T5,
                  typename T6>
        inline bool parse(T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5, T6& t6) const
        {
            if(!process(*(begin_ + 0), t0))
                return false;
            if(!process(*(begin_ + 1), t1))
        ...
        template <typename T, typename Allocator, template <typename, typename> class Sequence>
        inline bool parse(const col_range_t& range, Sequence<T, Allocator>& sequence) const
        {
        ...
        template <typename T, typename Comparator, typename Allocator>
        inline bool parse(const col_range_t& range, std::set<T, Comparator, Allocator>& set) const
        {
            if(!validate_column_range(range))
                return false;
        ...
        template <typename T, typename Container>
        inline bool parse(const col_range_t& range, std::queue<T, Container>& queue) const
        {
            if(!validate_column_range(range))
                return false;
        ...
        template <typename T, typename Container>
        inline bool parse(const col_range_t& range, std::stack<T, Container>& stack) const
        {
            if(!validate_column_range(range))
                return false;
        ...
        template <typename T, typename Container, typename Comparator>
        inline void
        parse_checked(std::priority_queue<T, Container, Comparator>& priority_queue) const
        {
    ...
    template <typename OutputIterator>
    inline bool extract_column_checked(const std::size_t& index, OutputIterator out) const
    {
    ...
    template <typename OutputIterator>
    inline bool
    extract_column(const row_range_t& row_range, const std::size_t& index, OutputIterator out) const
    {
    ...
    template <typename OutputIterator>
    inline bool extract_column(const std::size_t& index, OutputIterator out) const
    {
    ...
    template <typename OutputIterator0,
                               ...
                               OutputIterator4 out4) const
    {
        ...
        std::size_t max_index
            = std::max(index4, std::max(std::max(index0, index1), std::max(index2, index3)));
    ...
    inline std::size_t remove_empty_tokens() { return remove_empty_tokens(all_rows()); }

    inline void enforce_column_count(const row_range_t& row_range, const std::size_t& column_count)
    {
        if(row_range_invalid(row_range))
            return;
    ...
    inline std::size_t column_width(const std::size_t& col, const row_range_t& row_range) const
    {
    ...
    template <typename T>
    inline std::size_t
    accumulate_column(const std::size_t& col, const row_range_t& row_range, T& result) const
    {
        if(col > max_column_count_)
            return 0;
        else if(row_range_invalid(row_range))
    ...
    template <typename T, typename Predicate>
    inline std::size_t accumulate_column(const std::size_t& col, Predicate p, T& result) const
    {
    ...
    inline bool
    join_column(const std::size_t& col, const std::string& delimiter, std::string& result) const
    {
    ...
    inline bool
    sequential_partition(const row_range_t& row_range, TransitionPredicate p, Function f)
    {
        if(row_range_invalid(row_range))
            return false;
    ...
    bool
    load(unsigned char* buffer, const std::size_t buffer_size, const token_grid::options& options)
    {
        file_name_ = "";
...
private:
    ...
    {
        inline bool operator()(const range_t& r) const { return r.first == r.second; }
    };
    ...
    struct insufficient_number_of_columns
    {
        ...
        inline bool operator()(const row_type& row) const { return (num_of_cols != row.size()); }
    ...
    inline bool load()
    {
        ...
        if(!options_.support_dquotes)
        {
            ...
            strtk::split(
                ...
                strtk::functional_inserter(row_processor<multiple_char_delimiter_predicate>(
        ...
        } else
        {
            ...
            strtk::split(text_newline_predicate, buffer_, buffer_ + buffer_size_,
...
inline bool convert_string_range(
    const std::pair<std::string::const_iterator, std::string::const_iterator>& range, T& t)
{
    if(range.first == range.second)
        return false;
...
{
public:
    filter_on_match(const std::string* begin,
    ...
    inline void operator()(const std::string& s) const
    {
        for(const std::string* itr = begin_; end_ != itr; ++itr)
        {
            if((case_insensitive_ && (imatch((*itr).begin(), (*itr).end(), s.begin(), s.end())))
...
inline void skip_while_matching(Iterator& itr, const Iterator& end, const MatchPredicate& predicate)
{
    while(end != itr)
    {
        if(predicate(*itr))
...
struct size_equal_to
{
    ...
    inline bool operator()(const std::string& str) const { return length == str.size(); }
};
...
struct size_less_than
{
    ...
    inline bool operator()(const std::string& str) const { return str.size() < length; }
};
...
struct size_is_even
{
    ...
    inline bool operator()(const std::string& str) const { return 0 == (str.size() % 2); }
};
                  ...
                  T16& t16)
{
    ...
    const std::size_t parsed_token_count = split_n(delimiters, begin, end, token_count, token_list,
                                                   split_options::compress_delimiters);
...
#define strtk_cmpstmt(N)                                                                           \
    ...
    strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4) strtk_cmpstmt(5)
        strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8) strtk_cmpstmt(9) strtk_cmpstmt(10)
            strtk_cmpstmt(11) strtk_cmpstmt(12) strtk_cmpstmt(13) strtk_cmpstmt(14)
                  ...
                  T15& t15)
{
    ...
    strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4) strtk_cmpstmt(5)
        strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8) strtk_cmpstmt(9) strtk_cmpstmt(10)
            strtk_cmpstmt(11) strtk_cmpstmt(12) strtk_cmpstmt(13)
                  ...
                  T14& t14)
{
    ...
    strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4) strtk_cmpstmt(5)
        strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8) strtk_cmpstmt(9) strtk_cmpstmt(10)
            strtk_cmpstmt(11) strtk_cmpstmt(12)
                  ...
                  T11& t11)
{
    ...
    strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4) strtk_cmpstmt(5)
        strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8) strtk_cmpstmt(9)
                  ...
                  T10& t10)
{
    ...
    iterator_type_ptr itr = token_list;
    strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4) strtk_cmpstmt(5)
        strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8)
                  ...
                  T9& t9)
{
    ...
    iterator_type_ptr itr = token_list;
    strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4) strtk_cmpstmt(5)
        strtk_cmpstmt(6) strtk_cmpstmt(7)
...
inline bool
parse(const InputIterator begin, const InputIterator end, const std::string& delimiters, T& t)
{
    typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
    details::convert_type_assert<itr_type>();
...
namespace details {
                                  ...
                                  = split_options::compress_delimiters)
    {
        ...
        if(1 == delimiters.size())
            return split(single_delimiter_predicate<std::string::value_type>(delimiters[0]), begin,
                         end, out, split_option);
        else
...
#define strtk_def_parse_cont(Type)                                                                 \
    ...
    inline std::size_t parse(const InputIterator begin, const InputIterator end,                   \
                             ...
                             = split_options::compress_delimiters)                                 \
    {                                                                                              \
        return details::parse_impl(begin, end, delimiters, range_to_type_back_inserter(sequence),  \
                                   split_option);                                                  \
    }

strtk_def_parse_cont(std::vector) strtk_def_parse_cont(std::deque) strtk_def_parse_cont(std::list)

#undef strtk_def_parse_cont

    template <typename InputIterator, typename T, typename Comparator, typename Allocator>
    inline std::size_t
    parse(const InputIterator begin,
          const InputIterator end,
          const std::string& delimiters,
          std::set<T, Comparator, Allocator>& set,
          const split_options::type& split_option = split_options::compress_delimiters)
{
    return details::parse_impl(begin, end, delimiters, range_to_type_inserter(set), split_option);
}
...
inline std::size_t parse(const InputIterator begin,
                         ...
                         = split_options::compress_delimiters)
{
    return details::parse_impl(begin, end, delimiters, range_to_type_inserter(multiset),
                               split_option);
}
...
inline std::size_t parse(const InputIterator begin,
                         ...
                         = split_options::compress_delimiters)
{
    return details::parse_impl(begin, end, delimiters, range_to_type_push_inserter(queue),
                               split_option);
}
...
inline std::size_t parse(const InputIterator begin,
                         ...
                         = split_options::compress_delimiters)
{
    return details::parse_impl(begin, end, delimiters, range_to_type_push_inserter(stack),
                               split_option);
}

template <typename InputIterator, typename T, typename Container, typename Comparator>
inline std::size_t parse(const InputIterator begin,
                         const InputIterator end,
                         const std::string& delimiters,
                         std::priority_queue<T, Container, Comparator>& priority_queue,
                         ...
                         = split_options::compress_delimiters)
{
    return details::parse_impl(begin, end, delimiters, range_to_type_push_inserter(priority_queue),
                               split_option);
}
...
inline std::size_t parse(const std::pair<InputIterator, InputIterator>& range,
                         ...
                         = split_options::compress_delimiters)
{
    return details::parse_impl(range.first, range.second, delimiters, range_to_type_inserter(set),
                               split_option);
}
...
namespace details {
    ...
    private:
        ...
        template <typename T, typename Container, template <typename, typename> class SContainer>
                  ...
                  details::container_adder ca)
{
    ...
    if(1 == delimiters.size())
        split(single_delimiter_predicate<std::string::value_type>(delimiters[0]), begin, end,
                  ...
                  details::container_adder ca)
{
    ...
    iterator_type_ptr itr = token_list.begin();
    strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4) strtk_cmpstmt(5)
        strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8) return ca(itr, token_list.end());
}
                  ...
                  details::container_adder ca)
{
    ...
    iterator_type_ptr itr = token_list.begin();
    strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4) strtk_cmpstmt(5)
        strtk_cmpstmt(6) strtk_cmpstmt(7) return ca(itr, token_list.end());
}
                  ...
                  details::container_adder ca)
{
    ...
    iterator_type_ptr itr = token_list.begin();
    strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) return ca(itr, token_list.end());
}
...
#define strtk_def_parse_n_cont(Type)                                                               \
    ...
    inline std::size_t parse_n(                                                                    \
        const InputIterator begin, const InputIterator end, const std::string& delimiters,         \
    ...
    {                                                                                              \
        ...
        details::convert_type_assert<itr_type>();                                                  \
                                                                                                   \
        if(1 == delimiters.size())                                                                 \
            split_n(single_delimiter_predicate<std::string::value_type>(delimiters[0]), begin,     \
                    end, n, range_to_type_back_inserter(sequence), split_option);                  \
        else                                                                                       \
...
#undef strtk_def_parse_n_cont
        ...
        template <typename InputIterator, typename T, typename Comparator, typename Allocator>
        inline std::size_t
    parse_n(const InputIterator begin,
            const InputIterator end,
            const std::string& delimiters,
            const std::size_t& n,
            std::set<T, Comparator, Allocator>& set,
            const split_options::type& split_option = split_options::compress_delimiters)
{
    ...
    if(1 == delimiters.size())
        split_n(single_delimiter_predicate<std::string::value_type>(delimiters[0]), begin, end, n,
...
{
    return parse(
        to_ptr(data), to_ptr(data) + data.size(), delimiters, t1, t2, t3, t4, t5, t6, t7, t8, t9,
...
{
    return parse(
        to_ptr(data), to_ptr(data) + data.size(), delimiters, t1, t2, t3, t4, t5, t6, t7, t8,
...
{
    return parse(
        to_ptr(data), to_ptr(data) + data.size(), delimiters, t1, t2, t3, t4, t5, t6, t7,
...
{
    return parse(
        to_ptr(data), to_ptr(data) + data.size(), delimiters, t1, t2, t3, t4, t5, t6,
...
inline bool parse(
    const std::string& data, const std::string& delimiters, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5)
{
    return parse(
...
inline bool
parse(const std::string& data, const std::string& delimiters, T1& t1, T2& t2, T3& t3, T4& t4)
{
    return parse(
...
template <typename T1, typename T2, typename T3>
inline bool parse(const std::string& data, const std::string& delimiters, T1& t1, T2& t2, T3& t3)
{
    return parse(
...
template <typename T1, typename T2>
inline bool parse(const std::string& data, const std::string& delimiters, T1& t1, T2& t2)
{
    return parse(
...
template <typename T>
inline bool parse(const std::string& data, const std::string& delimiters, T& t)
{
    return parse(
...
#define strtk_def_parse_cont(Type)                                                                 \
    ...
    inline std::size_t parse(                                                                      \
        const std::string& data, const std::string& delimiters, Type<T, Allocator>& cont,          \
        const split_options::type& split_option = split_options::compress_delimiters)              \
    {                                                                                              \
        return parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, cont, split_option);    \
    }
...
#undef strtk_def_parse_cont
    ...
    parse(const std::string& data,
          ...
          const split_options::type& split_option = split_options::compress_delimiters)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, set, split_option);
}
...
inline std::size_t parse(const std::string& data,
                         ...
                         = split_options::compress_delimiters)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, multiset, split_option);
}
...
inline std::size_t parse(const std::string& data,
                         ...
                         = split_options::compress_delimiters)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, queue, split_option);
}
...
inline std::size_t parse(const std::string& data,
                         ...
                         = split_options::compress_delimiters)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, stack, split_option);
}
...
inline std::size_t parse(const std::string& data,
                         ...
                         = split_options::compress_delimiters)
{
    return parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, priority_queue,
                 split_option);
}

template <typename T, typename Allocator, template <typename, typename> class Sequence>
inline std::size_t parse(const int& argc,
                         char* argv[],
                         Sequence<T, Allocator>& sequence,
...
#define strtk_cmpstmt(N)                                                                           \
    ...
    strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4) strtk_cmpstmt(5)
        strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8) strtk_cmpstmt(9) return result;
}
...
template <typename T1,
          ...
          typename T8>
inline std::size_t
parse(const int& argc, char* argv[], T1& t1, T2& t2, T3& t3, T4& t4, T5& t5, T6& t6, T7& t7, T8& t8)

{
    ...
    std::size_t result = 0;
    strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4) strtk_cmpstmt(5)
        strtk_cmpstmt(6) strtk_cmpstmt(7) strtk_cmpstmt(8) return result;
}

template <typename T1, typename T2, typename T3, typename T4, typename T5, typename T6, typename T7>
inline std::size_t
parse(const int& argc, char* argv[], T1& t1, T2& t2, T3& t3, T4& t4, T5& t5, T6& t6, T7& t7)

{
    ...
    std::size_t result = 0;
    strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4) strtk_cmpstmt(5)
        strtk_cmpstmt(6) strtk_cmpstmt(7) return result;
}

template <typename T1, typename T2, typename T3, typename T4, typename T5, typename T6>
inline std::size_t
parse(const int& argc, char* argv[], T1& t1, T2& t2, T3& t3, T4& t4, T5& t5, T6& t6)
...
template <typename T1, typename T2, typename T3, typename T4, typename T5>
inline std::size_t parse(const int& argc, char* argv[], T1& t1, T2& t2, T3& t3, T4& t4, T5& t5)
{
    if(5 != argc)
        return 0;
    std::size_t result = 0;
...
template <typename T1, typename T2, typename T3, typename T4>
inline std::size_t parse(const int& argc, char* argv[], T1& t1, T2& t2, T3& t3, T4& t4)
{
    if(4 != argc)
        return 0;
    std::size_t result = 0;
    strtk_cmpstmt(1) strtk_cmpstmt(2) strtk_cmpstmt(3) strtk_cmpstmt(4) return result;
}
...
#define strtk_def_parse_n_seq(Type)                                                                \
    template <typename T, typename Allocator>                                                      \
    inline std::size_t parse_n(const std::string& data, const std::string& delimiters,             \
                               const std::size_t& n, Type<T, Allocator>& sequence,                 \
                               const split_options::type& split_option                             \
                               = split_options::compress_delimiters)                               \
    {                                                                                              \
        return parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n, sequence,          \
                       split_option);                                                              \
    }
...
#undef strtk_def_parse_n_seq
        ...
        template <typename T, typename Comparator, typename Allocator>
        inline std::size_t
    parse_n(const std::string& data,
            const std::string& delimiters,
            const std::size_t& n,
            std::set<T, Comparator, Allocator>& set,
            const split_options::type& split_option = split_options::compress_delimiters)
{
    return parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n, set, split_option);
}
...
inline std::size_t parse_n(const std::string& data,
                           ...
                           = split_options::compress_delimiters)
{
    return parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n, multiset, split_option);
}
...
inline std::size_t parse_n(const std::string& data,
                           ...
                           = split_options::compress_delimiters)
{
    return parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n, queue, split_option);
}
...
inline std::size_t parse_n(const std::string& data,
                           ...
                           = split_options::compress_delimiters)
{
    return parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n, stack, split_option);
}
...
inline std::size_t parse_n(const std::string& data,
                           ...
                           = split_options::compress_delimiters)
{
    return parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n, priority_queue,
                   split_option);
}
...
inline std::size_t parse_n(const std::string& data,
                           ...
                           = split_options::compress_delimiters)
{
    return parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n, out, split_option);
}
                       ...
                       T12& t12)
{
    ...
    if(data.empty() || delimiters.empty())
        return false;
    return strtk::parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, t1, t2, t3, t4, t5,
...
template <typename T1, typename T2, typename T3, typename T4, typename T5>
inline bool parse_line(
    std::ifstream& stream, const std::string& delimiters, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5)
{
    if(!stream)
        return false;
    std::string data;
...
template <typename T1, typename T2, typename T3, typename T4>
inline bool
parse_line(std::ifstream& stream, const std::string& delimiters, T1& t1, T2& t2, T3& t3, T4& t4)
{
    if(!stream)
        return false;
    std::string data;
    ...
    if(data.empty() || delimiters.empty())
        return false;
    return strtk::parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, t1, t2, t3, t4);
}

template <typename T1, typename T2, typename T3>
inline bool parse_line(std::ifstream& stream, const std::string& delimiters, T1& t1, T2& t2, T3& t3)
{
    if(!stream)
        return false;
    std::string data;
    ...
    if(data.empty() || delimiters.empty())
        return false;
    return strtk::parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, t1, t2, t3);
}

template <typename T1, typename T2>
inline bool parse_line(std::ifstream& stream, const std::string& delimiters, T1& t1, T2& t2)
{
    if(!stream)
        return false;
    std::string data;
    ...
    if(data.empty() || delimiters.empty())
        return false;
    return strtk::parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, t1, t2);
}

template <typename T1>
inline bool parse_line(std::ifstream& stream, const std::string& delimiters, T1& t1)
{
    if(!stream)
        return false;
    std::string data;
                              ...
                              = split_options::compress_delimiters)
{
    ...
    if(data.empty() || delimiters.empty())
        return false;
    return strtk::parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, sequence,
                        split_option);
}
                              ...
                              = split_options::compress_delimiters)
{
    ...
    if(data.empty() || delimiters.empty())
        return false;
    return strtk::parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, set, split_option);
}
                              ...
                              = split_options::compress_delimiters)
{
    ...
    if(data.empty() || delimiters.empty())
        return false;
    return strtk::parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, multiset,
                        split_option);
}
                              ...
                              = split_options::compress_delimiters)
{
    ...
    if(data.empty() || delimiters.empty())
        return false;
    return strtk::parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, queue, split_option);
}
                              ...
                              = split_options::compress_delimiters)
{
    ...
    if(data.empty() || delimiters.empty())
        return false;
    return strtk::parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, stack, split_option);
}
...

{
    ...
    if(data.empty() || delimiters.empty())
        return false;
    return strtk::parse(to_ptr(data), to_ptr(data) + data.size(), delimiters, priority_queue,
                        split_option);
}
                                ...
                                = split_options::compress_delimiters)
{
    ...
    if(data.empty() || delimiters.empty())
        return 0;
    return strtk::parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n, sequence,
                          split_option);
}
                                ...
                                = split_options::compress_delimiters)
{
    ...
    if(data.empty() || delimiters.empty())
        return 0;
    return strtk::parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n, set,
                          split_option);
}
                                ...
                                = split_options::compress_delimiters)
{
    ...
    if(data.empty() || delimiters.empty())
        return 0;
    return strtk::parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n, multiset,
                          split_option);
}
                                ...
                                = split_options::compress_delimiters)
{
    ...
    if(data.empty() || delimiters.empty())
        return 0;
    return strtk::parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n, queue,
                          split_option);
}
                                ...
                                = split_options::compress_delimiters)
{
    ...
    if(data.empty() || delimiters.empty())
        return 0;
    return strtk::parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n, stack,
                          split_option);
}
...

{
    ...
    if(data.empty() || delimiters.empty())
        return 0;
    return strtk::parse_n(to_ptr(data), to_ptr(data) + data.size(), delimiters, n, priority_queue,
                          split_option);
}
...
inline void construct(
    std::string& output, const std::string& delimiter, const T1& t1, const T2& t2, const T3& t3)
{
    output += type_to_string(t1);
    output += delimiter;
    output += type_to_string(t2);
...
template <typename T1, typename T2>
inline void construct(std::string& output, const std::string& delimiter, const T1& t1, const T2& t2)
{
    output += type_to_string(t1);
    output += delimiter;
    output += type_to_string(t2);
...
template <typename T, typename Allocator, template <typename, typename> class Sequence>
inline void
join(std::string& output, const std::string& delimiter, const Sequence<T, Allocator>& sequence)
{
...
inline void join(std::string& output, const std::string& delimiter, int argc, char* argv[])
{
    for(int i = 0; i < argc; ++i)
    {
        output += argv[i];
...
template <typename InputIterator>
inline std::string
join(const std::string& delimiter, const InputIterator begin, const InputIterator end)
{
    std::string output;
...
#define strtk_def_join_seq(Type)                                                                   \
    template <typename T, typename Allocator>                                                      \
    inline std::string join(const std::string& delimiter, const Type<T, Allocator>& sequence)      \
    {                                                                                              \
        if(sequence.empty())                                                                       \
            return "";                                                                             \
        else                                                                                       \
...
strtk_def_join_seq(std::vector) strtk_def_join_seq(std::deque) strtk_def_join_seq(std::list)
...
#undef strtk_def_join_seq
        ...
        template <typename T, typename Comparator, typename Allocator>
        inline std::string
    join(const std::string& delimiter, const std::set<T, Comparator, Allocator>& set)
{
    if(set.empty())
        return "";
    else
...
{
public:
    build_string(const std::size_t& initial_size = 64) { data_.reserve(initial_size); }
...
inline void replicate(const std::size_t& n, const std::string& str, std::string& output)
{
    if(0 == n)
        return;
...
strtk_def_brktz_cont(std::vector) strtk_def_brktz_cont(std::deque) strtk_def_brktz_cont(std::list)
    strtk_def_brktz_cont(std::queue) strtk_def_brktz_cont(std::stack)
...
inline std::size_t
inserter(Inserter ins, const InputIterator begin, const InputIterator end, OutputIterator out)
{
    std::size_t size = 0;
    InputIterator itr = begin;
...
template <typename InputIterator, typename OutputIterator>
inline void cut(const std::size_t& r0,
                const std::size_t& r1,
                const InputIterator begin,
                InputIterator end,
                OutputIterator out)
...
template <typename Allocator, template <typename, typename> class Sequence, typename OutputIterator>
inline void cut(const std::size_t& r0,
                const std::size_t& r1,
...
template <typename Iterator>
inline void
cut_inplace(const std::size_t& r0, const std::size_t& r1, const Iterator begin, const Iterator end)
{
    // static assert: InputIterator must be of type std::string
    Iterator itr = begin;
    while(end != itr)
...
template <typename Allocator, template <typename, typename> class Sequence>
inline void
cut(const std::size_t& r0, const std::size_t& r1, const Sequence<std::string, Allocator>& sequence)
{
...
#define strtk_register_rand_real_type_tag(T)                                                       \
    ...
    strtk_register_rand_int_type_tag(char) strtk_register_rand_int_type_tag(unsigned char)

        strtk_register_rand_int_type_tag(short) strtk_register_rand_int_type_tag(
            int) strtk_register_rand_int_type_tag(long)
            strtk_register_rand_int_type_tag(unsigned short) strtk_register_rand_int_type_tag(
                unsigned int) strtk_register_rand_int_type_tag(unsigned long)

                strtk_register_rand_real_type_tag(float) strtk_register_rand_real_type_tag(
                    double) strtk_register_rand_real_type_tag(long double)
...
template <typename Iterator, typename RandomNumberGenerator, typename OutputIterator>
inline void random_permutation(const Iterator begin,
                               const Iterator end,
                               RandomNumberGenerator& rng,
                               OutputIterator out)
{
...
template <typename Iterator, typename RandomNumberGenerator, typename OutputIterator>
inline bool random_combination(const Iterator begin,
                               const Iterator end,
                               std::size_t set_size,
                               RandomNumberGenerator& rng,
                               OutputIterator out)
...
template <typename Iterator, typename OutputIterator>
inline void random_combination(const Iterator begin,
                               const Iterator end,
...
template <typename Iterator, typename OutputIterator, typename RandomNumberGenerator>
inline std::size_t select_k_randomly(const Iterator begin,
                                     const Iterator end,
                                     const std::size_t k,
                                     OutputIterator out,
                                     RandomNumberGenerator& rng)
...
template <typename Iterator, typename OutputIterator, typename RandomNumberGenerator>
inline void select_1_randomly(const Iterator begin,
                              const Iterator end,
                              OutputIterator out,
                              RandomNumberGenerator& rng)
{
...
{
    if((first == last) || (first == k) || (last == k) || (last == details::next(first)))
        return false;
...
namespace details {
    ...
    template <typename Iterator>
    static inline void
    rotate_discontinuous(Iterator first1,
                         Iterator last1,
    ...
    inline void combine_discontinuous(Iterator first1,
                                      Iterator last1,
                                      typename std::iterator_traits<Iterator>::difference_type d1,
                                      Iterator first2,
                                      Iterator last2,
    ...
    template <typename Iterator, class Function>
    inline bool
    combine_discontinuous_conditional(Iterator first1,
                                      Iterator last1,
                                      typename std::iterator_traits<Iterator>::difference_type d1,
                                      Iterator first2,
                                      Iterator last2,
...
template <typename Iterator, typename Function>
inline void
for_each_combination(Iterator begin, Iterator end, const std::size_t& size, Function function)
{
    ...
    details::combine_discontinuous(begin, mid, std::distance(begin, mid), mid, end,
                                   std::distance(mid, end), func);
}

template <typename Iterator, typename Function>
inline void for_each_combination_conditional(Iterator begin,
                                             Iterator end,
                                             ...
                                             Function function)
{
    ...
    details::combine_discontinuous_conditional(begin, mid, std::distance(begin, mid), mid, end,
                                               std::distance(mid, end), func);
}
                                     ...
                                     const bool complete_index = true)
{
    ...
    index_list[k - 1]
        = index_list[k - 2] + static_cast<std::size_t>(n) - static_cast<std::size_t>(x);
...
class combination_iterator
    : public std::iterator<std::forward_iterator_tag, std::pair<Iterator, Iterator>, void, void>
{
public:
    typedef Iterator iterator;
    typedef const iterator const_iterator;
    ...
    {
        return (begin_ == itr.begin_) && (end_ == itr.end_) && (middle_ == itr.middle_);
    }

    inline bool operator!=(const combination_iterator& itr) const { return !operator==(itr); }
    ...
    case N:                                                                                        \
        return details::all_digits_check_impl<Iterator, N>::process(itr);
            ...
            case_stmt(0) case_stmt(1) case_stmt(2) case_stmt(3) case_stmt(4) case_stmt(5)
                case_stmt(6) case_stmt(7) case_stmt(8) case_stmt(9) case_stmt(10) case_stmt(11)
                    case_stmt(12) case_stmt(13) case_stmt(14) case_stmt(15) case_stmt(16)
                        case_stmt(17) case_stmt(18) case_stmt(19) default : return false;
#undef case_stmt
        }
    }
    ...
    template <typename T, typename Iterator>
    inline bool
    numeric_convert(const std::size_t n, const Iterator itr, T& t, const bool digit_check = false)
    {
        if(digit_check)
        {
    ...
    case N: {                                                                                      \
        ...
        return true;                                                                               \
            ...
            case_stmt(0) case_stmt(1) case_stmt(2) case_stmt(3) case_stmt(4) case_stmt(5)
                case_stmt(6) case_stmt(7) case_stmt(8) case_stmt(9) case_stmt(10) case_stmt(11)
                    case_stmt(12) case_stmt(13) case_stmt(14) case_stmt(15) case_stmt(16)
                        case_stmt(17) case_stmt(18) case_stmt(19) case_stmt(20) default
                : return false;
#undef case_stmt
        }
    }
    ...
    template <typename T, typename Iterator>
    inline bool
    signed_numeric_convert(Iterator itr, const Iterator end, T& t, const bool digit_check = false)
    {
    ...
    template <std::size_t N, typename T>
    inline bool signed_numeric_convert(const std::string& s, T& t, const bool digit_check = false)
    {
...
namespace binary {
...
#if(defined(__LITTLE_ENDIAN__)) || (defined(WIN32)) || (defined(_WIN32)) || (defined(__WIN32__))   \
    ...
    || (defined(__BYTE_ORDER__) && (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__))
        ...
        static inline unsigned int convert(const unsigned int v)
        {
            ...
            return ((v >> 24) & 0x000000FF) | ((v << 24) & 0x0000FF00) | ((v << 8) & 0x00FF0000)
                | ((v >> 8) & 0xFF000000);
        }
        ...
        static inline int convert_to_le(const int v) { return (is_big_endian()) ? convert(v) : v; }
...
#define strtk_def_rdr_opr(Type)                                                                    \
    ...
    {                                                                                              \
        ...
        strtk_def_rdr_opr(std::deque) strtk_def_rdr_opr(std::queue) strtk_def_rdr_opr(std::list)
#undef strtk_def_rdr_opr
        ...
        inline bool operator()(T& output, const std::size_t& size)
        {
            ...
            {
                bool result
                    = strtk::string_to_type_converter<char*, T>(buffer_, buffer_ + size, output);
                buffer_ += size;
        ...
        inline bool reset_to_mark() { return marker_.reset(amount_read_sofar_, buffer_); }
    ...
    private:
        ...
        private:
            ...
            struct selector_impl<T, strtk::details::yes_t>
            {
                template <typename Reader>
                static inline bool
                run(Reader& r, T& t, const bool perform_buffer_capacity_check = true)
                {
    ...
    public:
        ...
        template <typename T>
        inline bool
        operator()(const T* data, const uint32_t& length, const bool write_length = true)
        {
        ...
        template <typename T>
        inline bool
        operator()(const T* data, const uint64_t& length, const bool write_length = true)
        {
        ...
        template <typename T>
        inline bool
        operator()(const T* data, const uint16_t& length, const bool write_length = true)
        {
            return operator()(data, static_cast<uint32_t>(length), write_length);
        }
...
#define strtk_def_wrtr_opr(Type)                                                                   \
    ...
    {                                                                                              \
        ...
        strtk_def_wrtr_opr(std::deque) strtk_def_wrtr_opr(std::list) strtk_def_wrtr_opr(std::queue)
#undef strtk_def_wrtr_opr
        ...
        inline bool reset_to_mark() { return marker_.reset(amount_written_sofar_, buffer_); }
...
public:
    ...
    inline hex_to_number_sink& operator=(const std::pair<InputIterator, InputIterator>& s)
    {
        ...
        (*t_) = 0;
        valid_ = convert_hex_to_bin(buffer, buffer + (size - offset) + buffer_offset,
...
public:
    ...
    inline hex_to_string_sink& operator=(const std::pair<InputIterator, InputIterator>& s)
    {
        ...
        valid_ = convert_hex_to_bin(s.first + offset, s.second, const_cast<char*>(to_ptr(s_)));
        return (*this);
    }
...
#define strtk_def_parse_stl_cnt_prxy(Type)                                                         \
    ...
    strtk_def_parse_stl_cnt_prxy(std::vector) strtk_def_parse_stl_cnt_prxy(
        std::deque) strtk_def_parse_stl_cnt_prxy(std::list) strtk_def_parse_stl_cnt_prxy(std::queue)
        strtk_def_parse_stl_cnt_prxy(std::stack) strtk_def_parse_stl_cnt_prxy(std::priority_queue)
...
#define strtk_def_parse_n_stl_cnt_prxy(Type)                                                       \
    ...
    strtk_def_parse_n_stl_cnt_prxy(std::vector) strtk_def_parse_n_stl_cnt_prxy(
        std::deque) strtk_def_parse_n_stl_cnt_prxy(std::list)
        strtk_def_parse_n_stl_cnt_prxy(std::queue) strtk_def_parse_n_stl_cnt_prxy(
            std::stack) strtk_def_parse_n_stl_cnt_prxy(std::priority_queue)
    ...
    inline bool parse(InputIterator begin, InputIterator end)
    {
        if(container_)
        {
            if(std::numeric_limits<std::size_t>::max() == element_count_)
                return (details::parse_stl_container_proxy(begin, end, delimiters_, (*container_),
                                                           split_option_)
                        > 0);
            else
                return (details::parse_n_stl_container_proxy(
                            begin, end, delimiters_, element_count_, (*container_), split_option_)
                        == element_count_);
        } else
            return false;
    }
...
namespace text {
    inline std::string
    center(const std::size_t& width, const std::string::value_type& pad, const std::string& str)
    {
        if(str.size() >= width)
            return str;
    ...
    inline std::string
    left_align(const std::size_t& width, const std::string::value_type& pad, const std::string& str)
    {
        if(str.size() >= width)
            return str;
    ...
    template <typename T>
    inline std::string
    center(const std::size_t& width, const std::string::value_type& pad, const T& t)
    {
    ...
    template <typename T>
    inline std::string
    right_align(const std::size_t& width, const std::string::value_type& pad, const T& t)
    {
    ...
    template <typename T>
    inline std::string
    left_align(const std::size_t& width, const std::string::value_type& pad, const T& t)
    {
    ...
    inline bool is_lowercase_letter(const char c) { return (('a' <= c) && (c <= 'z')); }

    inline bool is_uppercase_letter(const char c) { return (('A' <= c) && (c <= 'Z')); }
    ...
    inline bool is_letter_or_digit(const char c) { return (is_letter(c) || is_digit(c)); }
...
namespace find_type {
    enum type { digits, letters, lowercase_letters, uppercase_letters, letters_digits };
}
...
namespace details {
    ...
    template <typename Iterator, typename Predicate>
    inline typename range_type<Iterator>::type
    find_exactly_n_consecutive_values(const std::size_t n,
                                      Predicate p,
                                      Iterator itr,
                                      const Iterator end,
    ...
    template <typename Iterator, typename Predicate>
    inline typename range_type<Iterator>::type find_atleast_n_consecutive_values(
        const std::size_t n, Predicate p, typename details::range_type<Iterator>::type range)
    {
    ...
    inline typename range_type<Iterator>::type find_n_consecutive_values(
        const std::size_t n, find_mode::type mode, Predicate p, Iterator itr, const Iterator end)
    {
        switch(mode)
        {
    ...
    template <typename Iterator, typename Predicate>
    inline bool match_n_consecutive_values(
        const std::size_t n, find_mode::type mode, Predicate p, Iterator itr, const Iterator end)
    {
        switch(mode)
        {
...
template <typename Iterator>
inline typename details::range_type<Iterator>::type
find_n_consecutive(const std::size_t n,
                   find_type::type type,
                   find_mode::type mode,
                   typename details::range_type<Iterator>::type range)
{
    ...
    case find_type::digits:
        return details::find_n_consecutive_values<Iterator>(n, mode, strtk::text::is_digit,
                                                            range.first, range.second);

    case find_type::letters:
        return details::find_n_consecutive_values<Iterator>(n, mode, strtk::text::is_letter,
                                                            range.first, range.second);
                                ...
                                typename details::range_type<Iterator>::type range)
{
    ...
    {
    case find_type::digits:
        return details::match_n_consecutive_values<Iterator>(n, mode, strtk::text::is_digit,
                                                             range.first, range.second);

    case find_type::letters:
        return details::match_n_consecutive_values<Iterator>(n, mode, strtk::text::is_letter,
                                                             range.first, range.second);
                                        ...
                                        const bool stateful_predicate = false)
{
    ...
    {
        range_type found_itr = details::find_exactly_n_consecutive_values<iterator_type, Predicate>(
...
inline std::size_t split_on_consecutive(const std::size_t& n,
                                        ...
                                        OutputIterator out)
{
    return split_on_consecutive<char*, OutputIterator>(n, type, mode, const_cast<char*>(begin),
                                                       const_cast<char*>(end), out);
}
...
inline std::size_t split_on_consecutive(const std::size_t& n,
                                        ...
                                        OutputIterator out)
{
    return split_on_consecutive<OutputIterator>(n, type, mode, reinterpret_cast<const char*>(begin),
...
inline std::size_t split_on_consecutive_n(const std::size_t& n,
                                          ...
                                          OutputIterator out)
{
    return split_on_consecutive_n<char*, OutputIterator>(n, m, type, mode, const_cast<char*>(begin),
                                                         const_cast<char*>(end), out);
}
                                        ...
                                        const bool stateful_predicate = false)
{
    return split_on_consecutive<Predicate, OutputIterator>(
        n, p, const_cast<char*>(begin), const_cast<char*>(end), out, stateful_predicate);
}
...
{
    return split_on_consecutive<Predicate, OutputIterator>(
        n, p, reinterpret_cast<const char*>(begin), reinterpret_cast<const char*>(end), out,
        stateful_predicate);
}
                                          ...
                                          OutputIterator out)
{
    return split_on_consecutive_n<Predicate, OutputIterator>(
        n, m, p, reinterpret_cast<const char*>(begin), reinterpret_cast<const char*>(end), out);
}
...
inline std::size_t split_on_consecutive_n(const std::size_t& n,
                                          ...
                                          OutputIterator out)
{
    return split_on_consecutive_n<Predicate, OutputIterator>(n, m, p, to_ptr(str),
                                                             to_ptr(str) + str.size(), out);
}
...
namespace details {
    ...
    public:
        ...
        inline bool operator()(InputIterator begin, InputIterator end) const
        {
            ...
            return strtk::match(to_ptr(s_), to_ptr(s_) + s_.size(), begin, end, zero_or_more,
                                zero_or_one);
        }
    ...
    namespace trim_details {
        ...
        {
            template <typename InputIterator>
            static bool execute(InputIterator begin,
                                InputIterator end,
...
inline details::expect_impl expect(const std::string& s) { return details::expect_impl(s); }

inline details::iexpect_impl iexpect(const std::string& s) { return details::iexpect_impl(s); }

inline details::like_impl like(const std::string& s) { return details::like_impl(s); }
...
namespace details {
    ...
    static const unsigned char rev_2digit_lut[] = { "0001020304050607080910111213141516171819"
    ...
    inline T pow10(T d, const int exponent)
    {
        static const double fract10[]
            = { 0.0,      1.0E+001, 1.0E+002, 1.0E+003, 1.0E+004, 1.0E+005, 1.0E+006, 1.0E+007,
                1.0E+008, 1.0E+009, 1.0E+010, 1.0E+011, 1.0E+012, 1.0E+013, 1.0E+014, 1.0E+015,
                1.0E+016, 1.0E+017, 1.0E+018, 1.0E+019, 1.0E+020, 1.0E+021, 1.0E+022, 1.0E+023,
                1.0E+024, 1.0E+025, 1.0E+026, 1.0E+027, 1.0E+028, 1.0E+029, 1.0E+030, 1.0E+031,
                1.0E+032, 1.0E+033, 1.0E+034, 1.0E+035, 1.0E+036, 1.0E+037, 1.0E+038, 1.0E+039,
                1.0E+040, 1.0E+041, 1.0E+042, 1.0E+043, 1.0E+044, 1.0E+045, 1.0E+046, 1.0E+047,
                1.0E+048, 1.0E+049, 1.0E+050, 1.0E+051, 1.0E+052, 1.0E+053, 1.0E+054, 1.0E+055,
                1.0E+056, 1.0E+057, 1.0E+058, 1.0E+059, 1.0E+060, 1.0E+061, 1.0E+062, 1.0E+063,
                1.0E+064, 1.0E+065, 1.0E+066, 1.0E+067, 1.0E+068, 1.0E+069, 1.0E+070, 1.0E+071,
                1.0E+072, 1.0E+073, 1.0E+074, 1.0E+075, 1.0E+076, 1.0E+077, 1.0E+078, 1.0E+079,
                1.0E+080, 1.0E+081, 1.0E+082, 1.0E+083, 1.0E+084, 1.0E+085, 1.0E+086, 1.0E+087,
                1.0E+088, 1.0E+089, 1.0E+090, 1.0E+091, 1.0E+092, 1.0E+093, 1.0E+094, 1.0E+095,
                1.0E+096, 1.0E+097, 1.0E+098, 1.0E+099, 1.0E+100, 1.0E+101, 1.0E+102, 1.0E+103,
                1.0E+104, 1.0E+105, 1.0E+106, 1.0E+107, 1.0E+108, 1.0E+109, 1.0E+110, 1.0E+111,
                1.0E+112, 1.0E+113, 1.0E+114, 1.0E+115, 1.0E+116, 1.0E+117, 1.0E+118, 1.0E+119,
                1.0E+120, 1.0E+121, 1.0E+122, 1.0E+123, 1.0E+124, 1.0E+125, 1.0E+126, 1.0E+127,
                1.0E+128, 1.0E+129, 1.0E+130, 1.0E+131, 1.0E+132, 1.0E+133, 1.0E+134, 1.0E+135,
                1.0E+136, 1.0E+137, 1.0E+138, 1.0E+139, 1.0E+140, 1.0E+141, 1.0E+142, 1.0E+143,
                1.0E+144, 1.0E+145, 1.0E+146, 1.0E+147, 1.0E+148, 1.0E+149, 1.0E+150, 1.0E+151,
                1.0E+152, 1.0E+153, 1.0E+154, 1.0E+155, 1.0E+156, 1.0E+157, 1.0E+158, 1.0E+159,
                1.0E+160, 1.0E+161, 1.0E+162, 1.0E+163, 1.0E+164, 1.0E+165, 1.0E+166, 1.0E+167,
                1.0E+168, 1.0E+169, 1.0E+170, 1.0E+171, 1.0E+172, 1.0E+173, 1.0E+174, 1.0E+175,
                1.0E+176, 1.0E+177, 1.0E+178, 1.0E+179, 1.0E+180, 1.0E+181, 1.0E+182, 1.0E+183,
                1.0E+184, 1.0E+185, 1.0E+186, 1.0E+187, 1.0E+188, 1.0E+189, 1.0E+190, 1.0E+191,
                1.0E+192, 1.0E+193, 1.0E+194, 1.0E+195, 1.0E+196, 1.0E+197, 1.0E+198, 1.0E+199,
                1.0E+200, 1.0E+201, 1.0E+202, 1.0E+203, 1.0E+204, 1.0E+205, 1.0E+206, 1.0E+207,
                1.0E+208, 1.0E+209, 1.0E+210, 1.0E+211, 1.0E+212, 1.0E+213, 1.0E+214, 1.0E+215,
                1.0E+216, 1.0E+217, 1.0E+218, 1.0E+219, 1.0E+220, 1.0E+221, 1.0E+222, 1.0E+223,
                1.0E+224, 1.0E+225, 1.0E+226, 1.0E+227, 1.0E+228, 1.0E+229, 1.0E+230, 1.0E+231,
                1.0E+232, 1.0E+233, 1.0E+234, 1.0E+235, 1.0E+236, 1.0E+237, 1.0E+238, 1.0E+239,
                1.0E+240, 1.0E+241, 1.0E+242, 1.0E+243, 1.0E+244, 1.0E+245, 1.0E+246, 1.0E+247,
                1.0E+248, 1.0E+249, 1.0E+250, 1.0E+251, 1.0E+252, 1.0E+253, 1.0E+254, 1.0E+255,
                1.0E+256, 1.0E+257, 1.0E+258, 1.0E+259, 1.0E+260, 1.0E+261, 1.0E+262, 1.0E+263,
                1.0E+264, 1.0E+265, 1.0E+266, 1.0E+267, 1.0E+268, 1.0E+269, 1.0E+270, 1.0E+271,
                1.0E+272, 1.0E+273, 1.0E+274, 1.0E+275, 1.0E+276, 1.0E+277, 1.0E+278, 1.0E+279,
                1.0E+280, 1.0E+281, 1.0E+282, 1.0E+283, 1.0E+284, 1.0E+285, 1.0E+286, 1.0E+287,
                1.0E+288, 1.0E+289, 1.0E+290, 1.0E+291, 1.0E+292, 1.0E+293, 1.0E+294, 1.0E+295,
                1.0E+296, 1.0E+297, 1.0E+298, 1.0E+299, 1.0E+300, 1.0E+301, 1.0E+302, 1.0E+303,
                1.0E+304, 1.0E+305, 1.0E+306, 1.0E+307, 1.0E+308 };
...
#define strtk_register_pod_type(T)                                                                 \
    ...
    strtk_register_pod_type(bool) strtk_register_pod_type(signed char) strtk_register_pod_type(char)
        strtk_register_pod_type(short) strtk_register_pod_type(int)
            strtk_register_pod_type(long int) strtk_register_pod_type(long long int)
                strtk_register_pod_type(unsigned char) strtk_register_pod_type(unsigned short)
                    strtk_register_pod_type(unsigned int) strtk_register_pod_type(unsigned long int)
                        strtk_register_pod_type(unsigned long long int)
                            strtk_register_pod_type(float) strtk_register_pod_type(double)
        ...
        strtk_register_signed_type_tag(short) strtk_register_signed_type_tag(
            ...
            strtk_register_real_type_tag(float) strtk_register_real_type_tag(
                ...
                strtk_register_byte_type_tag(unsigned char) strtk_register_byte_type_tag(
                    ...
                    strtk_register_hex_number_type_tag(hex_to_number_sink<short>) strtk_register_hex_number_type_tag(
                        hex_to_number_sink<
                            int>) strtk_register_hex_number_type_tag(hex_to_number_sink<long>)
                        strtk_register_hex_number_type_tag(hex_to_number_sink<long long>) strtk_register_hex_number_type_tag(
                            hex_to_number_sink<
                                unsigned short>) strtk_register_hex_number_type_tag(hex_to_number_sink<unsigned int>)
                            strtk_register_hex_number_type_tag(hex_to_number_sink<unsigned long>) strtk_register_hex_number_type_tag(
                                hex_to_number_sink<unsigned long long int>)

                                strtk_register_base64_type_tag(base64_to_number_sink<short>) strtk_register_base64_type_tag(
                                    base64_to_number_sink<
                                        int>) strtk_register_base64_type_tag(base64_to_number_sink<long>)
                                    strtk_register_base64_type_tag(base64_to_number_sink<long long>) strtk_register_base64_type_tag(
                                        base64_to_number_sink<
                                            unsigned short>) strtk_register_base64_type_tag(base64_to_number_sink<unsigned int>)
                                        strtk_register_base64_type_tag(base64_to_number_sink<unsigned long>) strtk_register_base64_type_tag(
                                            base64_to_number_sink<unsigned long long int>)
                                                ...
                                                strtk_register_stdstring_range_type_tag(signed char*) strtk_register_stdstring_range_type_tag(
                                                    ...
                                                    strtk_register_stdstring_range_type_tag(
                                                        ...
                                                        strtk_register_supported_iterator_type(char*) strtk_register_supported_iterator_type(
                                                            ...
                                                            strtk_register_supported_iterator_type(const char*) strtk_register_supported_iterator_type(
                                                                const signed char*)
                                                                strtk_register_supported_iterator_type(const unsigned char*) strtk_register_supported_iterator_type(
                                                                    std::string::iterator)
                                                                    strtk_register_supported_iterator_type(
                                                                        ...
                                                                        strtk_register_sequence_iterator_type(
                                                                            ...
                                                                            strtk_register_sequence_iterator_type(
                                                                                ...
                                                                                strtk_register_sink_type_tag(float) strtk_register_sink_type_tag(
                                                                                    double) strtk_register_sink_type_tag(long double)
                                                                                    strtk_register_sink_type_tag(
                                                                                        signed char) strtk_register_sink_type_tag(char) strtk_register_sink_type_tag(short) strtk_register_sink_type_tag(int) strtk_register_sink_type_tag(long) strtk_register_sink_type_tag(long long) strtk_register_sink_type_tag(unsigned char) strtk_register_sink_type_tag(unsigned short) strtk_register_sink_type_tag(unsigned int) strtk_register_sink_type_tag(unsigned long) strtk_register_sink_type_tag(unsigned long long int) strtk_register_sink_type_tag(std::
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               string)
                                                                                        ...
                                                                                        strtk_register_stl_container_to_string_conv_type_tag(
                                                                                            ...
                                                                                            strtk_register_stl_container_to_string_conv_type_tag(
                                                                                                ...
                                                                                                strtk_register_stl_container_to_string_conv_type_tag(
                                                                                                    long double)
                                                                                                    strtk_register_stl_container_to_string_conv_type_tag(signed char) strtk_register_stl_container_to_string_conv_type_tag(char) strtk_register_stl_container_to_string_conv_type_tag(short) strtk_register_stl_container_to_string_conv_type_tag(int) strtk_register_stl_container_to_string_conv_type_tag(long) strtk_register_stl_container_to_string_conv_type_tag(
                                                                                                        long long)
                                                                                                        strtk_register_stl_container_to_string_conv_type_tag(
                                                                                                            unsigned char)
                                                                                                            strtk_register_stl_container_to_string_conv_type_tag(unsigned short) strtk_register_stl_container_to_string_conv_type_tag(unsigned int) strtk_register_stl_container_to_string_conv_type_tag(unsigned long) strtk_register_stl_container_to_string_conv_type_tag(
                                                                                                                                        ...
                                                                                                                                        strtk_register_trim_type_tag(truncated_int<short>) strtk_register_trim_type_tag(
                                                                                                                                            ...
                                                                                                                                            strtk_register_trim_type_tag(
                                                                                                                                                ...
                                                                                                                                                strtk_register_trim_type_tag(
                                                                                                                                                    truncated_int<
                                                                                                                                                        unsigned char>)
                                                                                                                                                    strtk_register_trim_type_tag(truncated_int<
                                                                                                                                                                                 unsigned short>) strtk_register_trim_type_tag(truncated_int<unsigned int>)
                                                                                                                                                        strtk_register_trim_type_tag(
                                                                                                                                                            truncated_int<
                                                                                                                                                                unsigned long long int>)
                                                                                                                                                            ...
                                                                                                                                                            strtk_register_truncint_type_tag(
                                                                                                                                                                ...
                                                                                                                                                                strtk_register_truncint_type_tag(
                                                                                                                                                                    int)
                                                                                                                                                                    strtk_register_truncint_type_tag(long) strtk_register_truncint_type_tag(long long)
                                                                                                                                                                        strtk_register_truncint_type_tag(unsigned short) strtk_register_truncint_type_tag(unsigned int) strtk_register_truncint_type_tag(
                                                                                                                                                                            unsigned long)
                                                                                                                                                                            strtk_register_truncint_type_tag(
                                                                                                                                                                                unsigned long long int)
...
#define strtk_register_iostream_precision(T)                                                       \
    ...
    strtk_register_iostream_precision(float) strtk_register_iostream_precision(double)
...
#undef strtk_register_iostream_precision
    ...
    inline bool
    string_to_type_converter_impl(Iterator& itr, const Iterator end, Expect& t, expect_type_tag)
    {
        if(!t(itr, end))
            return false;
        itr = end;
    ...
    inline bool
    string_to_type_converter_impl(Iterator& itr, const Iterator end, Like& t, like_type_tag)
    {
        if(!t(itr, end))
            return false;
        itr = end;
    ...
    inline bool
    string_to_type_converter_impl(Iterator& itr, const Iterator end, InRange& t, inrange_type_tag)
    {
        if(!t(itr, end))
            return false;
        itr = end;
    ...
    inline bool
    string_to_type_converter_impl(Iterator& itr, const Iterator end, TrimToken& t, trim_type_tag)
    {
        if(!t(itr, end))
            return false;
        itr = end;
    ...
    inline bool
    string_to_type_converter_impl(Iterator& itr, const Iterator end, CaseToken& t, lcase_type_tag)
    {
        if(!t(itr, end))
            return false;
        itr = end;
    ...
    template <typename Iterator, typename CaseToken>
    inline bool
    string_to_type_converter_impl(Iterator& itr, const Iterator end, CaseToken& t, ucase_type_tag)
    {
        if(!t(itr, end))
            return false;
        itr = end;
    ...
    inline bool string_to_type_converter_impl(Iterator& itr,
                                              const Iterator end,
                                              Array& t,
                                              fillchararray_type_tag)
    {
        if(!t(itr, end))
            return false;
        itr = end;
...
#define case_stmt(N)                                                                               \
    case N:                                                                                        \
        if(strtk_likely((digit = (*itr++ - '0')) < 10))                                            \
            ...
            case_stmt(20) case_stmt(19) case_stmt(18) case_stmt(17) case_stmt(16) case_stmt(15)
                case_stmt(14) case_stmt(13) case_stmt(12) case_stmt(11) case_stmt(10) case_stmt(9)
                    case_stmt(8) case_stmt(7) case_stmt(6) case_stmt(5) case_stmt(4) case_stmt(3)
                        case_stmt(2) case 1
...
#undef case_stmt
        ...
        {
            if(!((t < numeric<T>::m10) || ((t == numeric<T>::m10) && (digit <= numeric<T>::ldpos))))
                return false;
        }
...
#define case_stmt(N)                                                                               \
    case N:                                                                                        \
        if(strtk_likely((digit = (*itr++ - '0')) < 10))                                            \
            ...
            case_stmt(19) case_stmt(18) case_stmt(17) case_stmt(16) case_stmt(15) case_stmt(14)
                case_stmt(13) case_stmt(12) case_stmt(11) case_stmt(10) case_stmt(9) case_stmt(8)
                    case_stmt(7) case_stmt(6) case_stmt(5) case_stmt(4) case_stmt(3)
                        case_stmt(2) case 1
                : if(strtk_unlikely((digit = (*itr - '0')) >= 10)) return false;
#undef case_stmt
        }
    ...
    template <typename Iterator, typename T>
    inline bool
    string_to_type_converter_impl_ref(Iterator& itr, const Iterator end, T& result, signed_type_tag)
    {
        if(itr == end)
            return false;
...
#define strtk_process_digit                                                                        \
    ...
    {                                                                                              \
        ...
        break;                                                                                     \
            ...
            case 4:
                strtk_process_digit case 3 : strtk_process_digit case 2 : strtk_process_digit case 1
...
#undef strtk_process_digit
    ...
    template <typename Iterator, typename T>
    inline bool
    string_to_type_converter_impl(Iterator& itr_external, const Iterator end, T& t, real_type_tag)
    {
        if(end == itr_external)
            return false;
...
#undef strtk_unlikely
    ...
    template <typename Iterator, typename T>
    inline bool
    string_to_type_converter_impl(Iterator& itr, const Iterator end, T& t, byte_type_tag)
    {
    ...
    template <typename Iterator>
    inline bool
    string_to_type_converter_impl(Iterator& itr, const Iterator end, bool& t, bool_type_tag)
    {
        if(1 != std::distance(itr, end))
            return false;
        t = (('0' == (*itr)) ? false : true);
    ...
    inline bool
    string_to_type_converter_impl(Iterator& itr, const Iterator end, SinkType& t, sink_type_tag)
    {
        if(!t.parse(itr, end))
            return false;
        itr = end;
...
#define define_tsci_type(Type, ReType)                                                             \
    ...
    define_tsci_type(short, unsigned short) define_tsci_type(int, unsigned int)
        define_tsci_type(long, unsigned long) define_tsci_type(long long, unsigned long long)

#undef define_tsci_type

            template <typename T>
            inline bool type_to_string_converter_impl(T valuex,
                                                      std::string& result,
                                                      signed_type_tag)
    {
    ...
    inline bool
    type_to_string_converter_impl(const std::string& value, std::string& result, stdstring_type_tag)
    {
        result = value;
        return true;
    }
    ...
    template <typename Iterator>
    inline bool type_to_string_converter_impl(const std::pair<Iterator, Iterator>& range,
                                              std::string& result,
    ...
    inline bool
    type_to_string_converter_impl(const STLContainerType&, std::string&, stl_seq_type_tag)
    {
...
#define strtk_register_type_name(Type)                                                             \
    ...
    strtk_register_type_name(signed char) strtk_register_type_name(unsigned char)
        strtk_register_type_name(short) strtk_register_type_name(int) strtk_register_type_name(long)
            strtk_register_type_name(long long) strtk_register_type_name(unsigned short)
                strtk_register_type_name(unsigned int) strtk_register_type_name(unsigned long)
                    strtk_register_type_name(unsigned long long int)
                        strtk_register_type_name(double) strtk_register_type_name(float)
...
#undef strtk_register_type_name
    ...
    inline std::string type_name(const std::pair<T1, T2>& p)
    {
        static const std::string s
            = std::string("std::pair<" + type_name(p.first) + "," + type_name(p.second) + ">");
        return s;
    }
...
{
    static const std::string s
        = details::type_name<T>() + std::string("[") + type_to_string(N) + std::string("]");
    return s;
}
...
strtk_register_sequence_type_name(std::vector) strtk_register_sequence_type_name(std::deque)
    strtk_register_sequence_type_name(std::list) strtk_register_set_type_name(std::set)
...
public:
    ...
    inline bool imatch(const std::string& s) const { return strtk::imatch(s_, s); }

    inline bool imatch(const ext_string& es) const { return strtk::imatch(s_, es.s_); }
    ...
    inline ext_string& remove_leading(const std::string& removal_set)
    {
        if(removal_set.empty())
            ...
            strtk::remove_leading(
                single_delimiter_predicate<std::string::value_type>(removal_set[0]), s_);
        else
...
namespace fileio {
    ...
    inline bool
    write_file(const std::string& file_name, char* buffer, const std::size_t& buffer_size)
    {
    ...
    namespace details {
        ...
        inline bool write_pod_proxy(std::ofstream& stream, const T& t)
        {
            return (
                false
                == stream
                       .write(reinterpret_cast<char*>(&t), static_cast<std::streamsize>(sizeof(T)))
                       .fail());
        }
    } // namespace details
    ...
    inline bool read_pod(std::ifstream& stream,
                         ...
                         T10& t10)
    {
        return details::read_pod_proxy(stream, t1) && details::read_pod_proxy(stream, t2)
            && details::read_pod_proxy(stream, t3) && details::read_pod_proxy(stream, t4)
            && details::read_pod_proxy(stream, t5) && details::read_pod_proxy(stream, t6)
            && details::read_pod_proxy(stream, t7) && details::read_pod_proxy(stream, t8)
            && details::read_pod_proxy(stream, t9) && details::read_pod_proxy(stream, t10);
    }
    ...
    template <typename T1,
              ...
              typename T8>
    inline bool
    read_pod(std::ifstream& stream, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5, T6& t6, T7& t7, T8& t8)
    {
        return details::read_pod_proxy(stream, t1) && details::read_pod_proxy(stream, t2)
            && details::read_pod_proxy(stream, t3) && details::read_pod_proxy(stream, t4)
            && details::read_pod_proxy(stream, t5) && details::read_pod_proxy(stream, t6)
    ...
    template <typename T1,
              ...
              typename T7>
    inline bool
    read_pod(std::ifstream& stream, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5, T6& t6, T7& t7)
    {
        return details::read_pod_proxy(stream, t1) && details::read_pod_proxy(stream, t2)
            && details::read_pod_proxy(stream, t3) && details::read_pod_proxy(stream, t4)
            && details::read_pod_proxy(stream, t5) && details::read_pod_proxy(stream, t6)
    ...
    template <typename T1, typename T2, typename T3, typename T4, typename T5, typename T6>
    inline bool read_pod(std::ifstream& stream, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5, T6& t6)
    {
        return details::read_pod_proxy(stream, t1) && details::read_pod_proxy(stream, t2)
            && details::read_pod_proxy(stream, t3) && details::read_pod_proxy(stream, t4)
            && details::read_pod_proxy(stream, t5) && details::read_pod_proxy(stream, t6);
    ...
    template <typename T1, typename T2, typename T3, typename T4, typename T5>
    inline bool read_pod(std::ifstream& stream, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5)
    {
        return details::read_pod_proxy(stream, t1) && details::read_pod_proxy(stream, t2)
            && details::read_pod_proxy(stream, t3) && details::read_pod_proxy(stream, t4)
    ...
    template <typename T, typename Allocator, template <typename, typename> class Sequence>
    inline bool
    read_pod(std::ifstream& stream, const std::size_t& count, Sequence<T, Allocator>& sequence)
    {
        T t;
    ...
    inline bool write_pod(std::ofstream& stream,
                          ...
                          const T10& t10)
    {
        return details::write_pod_proxy(stream, t1) && details::write_pod_proxy(stream, t2)
            && details::write_pod_proxy(stream, t3) && details::write_pod_proxy(stream, t4)
            && details::write_pod_proxy(stream, t5) && details::write_pod_proxy(stream, t6)
            && details::write_pod_proxy(stream, t7) && details::write_pod_proxy(stream, t8)
            && details::write_pod_proxy(stream, t9) && details::write_pod_proxy(stream, t10);
    }
    ...
    template <typename T1, typename T2, typename T3, typename T4, typename T5>
    inline bool write_pod(
        std::ofstream& stream, const T1& t1, const T2& t2, const T3& t3, const T4& t4, const T5& t5)
    {
        return details::write_pod_proxy(stream, t1) && details::write_pod_proxy(stream, t2)
            && details::write_pod_proxy(stream, t3) && details::write_pod_proxy(stream, t4)
    ...
    template <typename T1, typename T2, typename T3, typename T4>
    inline bool
    write_pod(std::ofstream& stream, const T1& t1, const T2& t2, const T3& t3, const T4& t4)
    {
        return details::write_pod_proxy(stream, t1) && details::write_pod_proxy(stream, t2)
            && details::write_pod_proxy(stream, t3) && details::write_pod_proxy(stream, t4);
    }
    ...
    template <typename T1, typename T2, typename T3>
    inline bool write_pod(std::ofstream& stream, const T1& t1, const T2& t2, const T3& t3)
    {
        return details::write_pod_proxy(stream, t1) && details::write_pod_proxy(stream, t2)
    ...
    template <typename T, typename Allocator, template <typename, typename> class Sequence>
    inline bool write_pod(std::ofstream& stream, const Sequence<T, Allocator>& sequence)
    {
    ...
    template <typename T, typename Comparator, typename Allocator>
    inline bool write_pod(std::ofstream& stream, const std::set<T, Comparator, Allocator>& set)
    {
                               ...
                               const std::size_t& buffer_size)
    {
        ...
        return read_at_offset(stream, offset, const_cast<char*>(to_ptr(buffer)), buffer_size);
    }
...
inline unsigned char* read_pod(
    unsigned char* data, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5, T6& t6, T7& t7, T8& t8, T9& t9)
{
    ::memcpy(&t1, data, sizeof(T1));
    data += sizeof(T1);
    ::memcpy(&t2, data, sizeof(T2));
...
read_pod(unsigned char* data, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5, T6& t6, T7& t7, T8& t8)
{
    ::memcpy(&t1, data, sizeof(T1));
    data += sizeof(T1);
    ::memcpy(&t2, data, sizeof(T2));
...
template <typename T1, typename T2, typename T3, typename T4, typename T5, typename T6, typename T7>
inline unsigned char*
read_pod(unsigned char* data, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5, T6& t6, T7& t7)
{
    ::memcpy(&t1, data, sizeof(T1));
    data += sizeof(T1);
    ::memcpy(&t2, data, sizeof(T2));
...
template <typename T1, typename T2, typename T3, typename T4, typename T5, typename T6>
inline unsigned char* read_pod(unsigned char* data, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5, T6& t6)
{
    ::memcpy(&t1, data, sizeof(T1));
    data += sizeof(T1);
    ::memcpy(&t2, data, sizeof(T2));
...
template <typename T1, typename T2, typename T3, typename T4, typename T5>
inline unsigned char* read_pod(unsigned char* data, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5)
{
    ::memcpy(&t1, data, sizeof(T1));
    data += sizeof(T1);
    ::memcpy(&t2, data, sizeof(T2));
...
template <typename T1, typename T2, typename T3, typename T4>
inline unsigned char* read_pod(unsigned char* data, T1& t1, T2& t2, T3& t3, T4& t4)
{
    ::memcpy(&t1, data, sizeof(T1));
    data += sizeof(T1);
    ::memcpy(&t2, data, sizeof(T2));
...
template <typename T, typename Allocator, template <typename, typename> class Sequence>
inline unsigned char*
read_pod(unsigned char* data, const std::size_t& n, const Sequence<T, Allocator>& sequence)
{
...
template <typename T, typename Comparator, typename Allocator>
inline unsigned char*
read_pod(unsigned char* data, const std::size_t& n, const std::set<T, Comparator, Allocator>& set)
{
    T* ptr = reinterpret_cast<T>(data);
...
template <typename T1, typename T2, typename T3, typename T4, typename T5>
inline unsigned char*
write_pod(unsigned char* data, const T1& t1, const T2& t2, const T3& t3, const T4& t4, const T5& t5)
{
    ::memcpy(data, &t1, sizeof(T1));
    data += sizeof(T1);
    ::memcpy(data, &t2, sizeof(T2));
...
template <typename T1, typename T2, typename T3, typename T4>
inline unsigned char*
write_pod(unsigned char* data, const T1& t1, const T2& t2, const T3& t3, const T4& t4)
{
    ::memcpy(data, &t1, sizeof(T1));
    data += sizeof(T1);
    ::memcpy(data, &t2, sizeof(T2));
...
template <typename T1, typename T2, typename T3>
inline unsigned char* write_pod(unsigned char* data, const T1& t1, const T2& t2, const T3& t3)
{
    ::memcpy(data, &t1, sizeof(T1));
    data += sizeof(T1);
    ::memcpy(data, &t2, sizeof(T2));
...
template <typename T, typename Allocator, template <typename, typename> class Sequence>
inline unsigned char* write_pod(unsigned char* data, const Sequence<T, Allocator>& sequence)
{
...
template <typename T, typename Comparator, typename Allocator>
inline unsigned char* write_pod(unsigned char* data, const std::set<T, Comparator, Allocator>& set)
{
    T* ptr = reinterpret_cast<T>(data);
...
namespace trie {
    ...
    public:
        ...
        inline void insert(const key_iterator_t begin, const key_iterator_t end, const value_t& v)
        {
            if(0 == std::distance(begin, end))
                return;
        ...
        template <typename key_iterator_t>
        inline bool find(const key_iterator_t begin, const key_iterator_t end, value_t& v) const
        {
    ...
    template <typename Value>
    inline bool
    find(prefix<std::string::const_iterator, Value>& trie, const std::string& key, Value& v)
    {
        return trie.find(key.begin(), key.end(), v);
    }

    template <typename Value>
    inline bool find(prefix<std::string::const_iterator, Value>& trie, const char* key, Value& v)
    {
    ...
    template <typename Value>
    inline bool find_prefix(prefix<std::string::const_iterator, Value>& trie, const char* key)
    {
...
namespace bloom {
    ...
    public:
        ...
        {
            return reader(minimum_size) && reader(maximum_size) && reader(minimum_number_of_hashes)
                && reader(maximum_number_of_hashes) && reader(projected_element_count)
                && reader(false_positive_probability) && reader(random_seed);
        }

        inline bool operator()(strtk::binary::writer& writer)
        {
            return writer(minimum_size) && writer(maximum_size) && writer(minimum_number_of_hashes)
                && writer(maximum_number_of_hashes) && writer(projected_element_count)
    ...
    public:
        ...
        inline bool write_to_file(const std::string& file_name) const
        {
            if(0 == table_size_)
                return false;
            const std::size_t buffer_size = sizeof(salt_count_) + sizeof(table_size_)
                + sizeof(raw_table_size_) + sizeof(projected_element_count_)
                + sizeof(inserted_element_count_) + sizeof(random_seed_)
                + sizeof(desired_false_positive_probability_) + salt_count_ * sizeof(bloom_type)
            ...
            writer.reset(true);
            bool result = writer(salt_count_) && writer(table_size_) && writer(raw_table_size_)
                && writer(projected_element_count_) && writer(inserted_element_count_)
                && writer(random_seed_) && writer(desired_false_positive_probability_)
        ...
        inline bool read_from_file(const std::string& file_name)
        {
            ...
            bool result = reader(salt_count_) && reader(table_size_) && reader(raw_table_size_)
                && reader(projected_element_count_) && reader(inserted_element_count_)
                && reader(random_seed_) && reader(desired_false_positive_probability_)
    ...
    protected:
        ...
        inline bloom_type
        hash_ap(const unsigned char* begin, std::size_t remaining_length, bloom_type hash) const
        {
            const unsigned char* itr = begin;
            unsigned int loop = 0;
    ...
    public:
        ...
        inline virtual unsigned long long int size() const { return size_list.back(); }
        ...
        inline bool compress(const double& percentage)
        {
            ...
            new_table_size
                -= (((new_table_size % bits_per_char) != 0) ? (new_table_size % bits_per_char) : 0);
...
template <typename Iterator>
inline unsigned int
hash(const Iterator itr, std::size_t length, unsigned int seed = details::hash_seed)
{
...
namespace util {
    ...
    public:
        ...
        inline void assign(Function& f)
        {
            ...
            function_holder_ = construct<Function, type_size <= function_holder_buffer_size>::type(
...
#define strtk_register_attribute_type_tag(T)                                                       \
    ...
    strtk_register_attribute_type_tag(unsigned short) strtk_register_attribute_type_tag(
        unsigned int) strtk_register_attribute_type_tag(unsigned long)
        strtk_register_attribute_type_tag(unsigned long long int)
            strtk_register_attribute_type_tag(short) strtk_register_attribute_type_tag(int)
                strtk_register_attribute_type_tag(long) strtk_register_attribute_type_tag(long long)
    ...
    template <typename T>
    inline bool type_to_string_converter_impl(const strtk::util::attribute<T>& attrib,
                                              std::string& result,
                                              attribute_type_tag)
    {
...
#undef strtk_register_attribute_type_tag
    ...
    inline bool string_to_type_converter_impl(Iterator& itr,
                                              const Iterator end,
                                              strtk::util::semantic_action_impl& result,
                                              semantic_action_type_tag)
    {
        return result(itr, end);
    }

    inline bool type_to_string_converter_impl(const strtk::util::semantic_action_impl&,
                                              std::string& result,
                                              semantic_action_type_tag)
    {
...
namespace util {
    ...
    public:
        ...
        {
            return (0 != type_holder_) && (0 != v.type_holder_) && (type_holder_ == v.type_holder_);
        }
            ...
            if(&v != this)
            {
                if(0 != v.type_holder_)
                {
                    std::copy(v.type_holder_buffer_,
                              v.type_holder_buffer_ + type_holder_buffer_size, type_holder_buffer_);
        ...
        inline void assign(T& t)
        {
            ...
            type_holder_
                = construct<T, type_size <= type_holder_buffer_size>::type(t, type_holder_buffer_);
        }
...
#define strtk_def_mkky_list(Type)                                                                  \
    ...
    strtk_def_mkky_list(std::vector) strtk_def_mkky_list(std::deque) strtk_def_mkky_list(std::list)
...
#undef strtk_def_mkky_list
    ...
    template <typename Predicate, typename Key, typename T, typename Comparator, typename Allocator>
    inline void delete_if(const Predicate& p, std::map<Key, T*, Comparator, Allocator>& cont)
    {
        typename std::map<Key, T*, Comparator, Allocator>::iterator itr = cont.begin();
    ...
    template <typename Predicate, typename Key, typename T, typename Comparator, typename Allocator>
    inline void delete_if(const Predicate& p, std::multimap<Key, T*, Comparator, Allocator>& cont)
    {
        typename std::multimap<Key, T*, Comparator, Allocator>::iterator itr = cont.begin();
    ...
    inline void delete_if(const Predicate& p, std::set<T*, Comparator, Allocator>& cont)
    {
        typename std::set<T*, Comparator, Allocator>::iterator itr = cont.begin();
    ...
    template <typename Predicate, typename T, typename Comparator, typename Allocator>
    inline void delete_if(const Predicate& p, std::multiset<T*, Comparator, Allocator>& cont)
    {
        typename std::multiset<T*, Comparator, Allocator>::iterator itr = cont.begin();
    ...
    template <typename T, typename Allocator, template <typename, typename> class Sequence>
    inline void
    push_back(Sequence<T, Allocator>& sequence, const T& v1, const T& v2, const T& v3, const T& v4)
    {
        sequence.push_back(v1);
        sequence.push_back(v2);
        sequence.push_back(v3);
    ...
    template <typename T, typename Allocator, template <typename, typename> class Sequence>
    inline void push_back(Sequence<T, Allocator>& sequence, const T& v1, const T& v2, const T& v3)
    {
        sequence.push_back(v1);
        sequence.push_back(v2);
        sequence.push_back(v3);
    ...
    template <typename T, typename Allocator, template <typename, typename> class Sequence>
    inline void push_back(Sequence<T, Allocator>& sequence, const T& v1, const T& v2)
    {
        sequence.push_back(v1);
        sequence.push_back(v2);
    }
    ...
    inline void push_back(
        std::set<T, Comparator, Allocator>& set, const T& v1, const T& v2, const T& v3, const T& v4)
    {
        set.insert(v1);
        set.insert(v2);
        set.insert(v3);
    ...
    template <typename T, typename Comparator, typename Allocator>
    inline void
    push_back(std::set<T, Comparator, Allocator>& set, const T& v1, const T& v2, const T& v3)
    {
        set.insert(v1);
        set.insert(v2);
        set.insert(v3);
    ...
    template <typename T, typename Comparator, typename Allocator>
    inline void push_back(std::set<T, Comparator, Allocator>& set, const T& v1, const T& v2)
    {
        set.insert(v1);
        set.insert(v2);
    }
    ...
    template <typename T, typename Comparator, typename Allocator>
    inline void
    push_back(std::multiset<T, Comparator, Allocator>& set, const T& v1, const T& v2, const T& v3)
    {
        set.insert(v1);
        set.insert(v2);
        set.insert(v3);
    ...
    template <typename T, typename Comparator, typename Allocator>
    inline void push_back(std::multiset<T, Comparator, Allocator>& set, const T& v1, const T& v2)
    {
        set.insert(v1);
        set.insert(v2);
    }
...
#define strtk_def_clear_seq(Type)                                                                  \
    ...
    strtk_def_clear_seq(std::vector) strtk_def_clear_seq(std::deque) strtk_def_clear_seq(std::list)
        strtk_def_clear_seq(std::priority_queue) strtk_def_clear_seq(std::stack)
...
namespace details {
    ...
    class column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9>
        : public column_selector_base<column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9>,
                                      10>
    {
    public:
        typedef column_selector_base<column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9>,
                                     10>
            csb_t;
    ...
    public:
        ...
        column_selector_impl(
            const column_list_t& column_list, T1& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5)
            : csb_t(column_list)
        {
            csb_t::cvl().register_value(t0);
            csb_t::cvl().register_value(t1);
    ...
    public:
        ...
        column_selector_impl(
            const column_list_t& column_list, T1& t0, T1& t1, T2& t2, T3& t3, T4& t4)
            : csb_t(column_list)
        {
            csb_t::cvl().register_value(t0);
            csb_t::cvl().register_value(t1);
    ...
    template <typename T0>
    class column_selector_impl<T0> : public column_selector_base<column_selector_impl<T0>, 1>
    {
    public:
...
column_list(const std::size_t& idx0, const std::size_t& idx1, const std::size_t& idx2)
{
...
inline details::column_list_impl<2> column_list(const std::size_t& idx0, const std::size_t& idx1)
{
...
{
    return column_list(idx[0], idx[1], idx[2], idx[3], idx[4], idx[5], idx[6], idx[7], idx[8],
                       idx[9], idx[10], idx[11]);
}
...
{
    return column_list(idx[0], idx[1], idx[2], idx[3], idx[4], idx[5], idx[6], idx[7]);
}
...
column_selector(const details::column_list_impl<11>& col_list,
                ...
                T11& t11)
{
    return details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11>(
...
column_selector(const details::column_list_impl<11>& col_list,
                ...
                T10& t10)
{
    return details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10>(
...
column_selector(const details::column_list_impl<8>& col_list,
                ...
                T7& t7)
{
    return details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7>(col_list, t0, t1, t2, t3,
                                                                         t4, t5, t6, t7);
}
...
column_selector(const details::column_list_impl<7>& col_list,
                ...
                T6& t6)
{
    return details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6>(col_list, t0, t1, t2, t3, t4,
                                                                     t5, t6);
}

template <typename T0, typename T1, typename T2, typename T3, typename T4, typename T5>
inline typename details::column_selector_impl<T0, T1, T2, T3, T4, T5> column_selector(
    const details::column_list_impl<6>& col_list, T0& t0, T1& t1, T2& t2, T3& t3, T4& t4, T5& t5)
{
...
inline typename details::column_selector_impl<T0, T1, T2, T3, T4> column_selector(
    const details::column_list_impl<5>& col_list, T0& t0, T1& t1, T2& t2, T3& t3, T4& t4)
{
...
column_selector(const details::column_list_impl<4>& col_list, T0& t0, T1& t1, T2& t2, T3& t3)
{
...
column_selector(const details::column_list_impl<3>& col_list, T0& t0, T1& t1, T2& t2)
{
...
column_selector(const details::column_list_impl<2>& col_list, T0& t0, T1& t1)
{
                          ...
                          T11& t11)
{
    strtk_parse_columns_impl(12) strtk_parse_col_token(0) strtk_parse_col_token(1)
        strtk_parse_col_token(2) strtk_parse_col_token(3) strtk_parse_col_token(4)
            strtk_parse_col_token(5) strtk_parse_col_token(6) strtk_parse_col_token(7)
                strtk_parse_col_token(8) strtk_parse_col_token(9) strtk_parse_col_token(10)
                          ...
                          T7& t7)
{
    strtk_parse_columns_impl(8) strtk_parse_col_token(0) strtk_parse_col_token(1)
        strtk_parse_col_token(2) strtk_parse_col_token(3) strtk_parse_col_token(4)
            strtk_parse_col_token(5) strtk_parse_col_token(6) strtk_parse_col_token(7) return true;
}
...
template <typename InputIterator, typename T0, typename T1, typename T2, typename T3, typename T4>
inline bool parse_columns(const InputIterator begin,
                          const InputIterator end,
                          const std::string& delimiters,
                          ...
                          T4& t4)
{
    strtk_parse_columns_impl(5) strtk_parse_col_token(0) strtk_parse_col_token(1)
        strtk_parse_col_token(2) strtk_parse_col_token(3) strtk_parse_col_token(4) return true;
}

template <typename InputIterator, typename T0, typename T1, typename T2, typename T3>
inline bool parse_columns(const InputIterator begin,
                          const InputIterator end,
                          const std::string& delimiters,
...
inline bool parse_columns(const InputIterator begin,
                          ...
                          T1& t1)
{
    strtk_parse_columns_impl(2) strtk_parse_col_token(0) strtk_parse_col_token(1) return true;
}
...
inline bool parse_columns(const InputIterator begin,
                          ...
                          Sequence<T, Allocator>& seq)
{
    strtk_parse_columns_impl(12) strtk_parse_col_token_seq(0) strtk_parse_col_token_seq(1)
        strtk_parse_col_token_seq(2) strtk_parse_col_token_seq(3) strtk_parse_col_token_seq(4)
            strtk_parse_col_token_seq(5) strtk_parse_col_token_seq(6) strtk_parse_col_token_seq(7)
...
inline bool parse_columns(const InputIterator begin,
                          ...
                          Sequence<T, Allocator>& seq)
{
    strtk_parse_columns_impl(11) strtk_parse_col_token_seq(0) strtk_parse_col_token_seq(1)
        strtk_parse_col_token_seq(2) strtk_parse_col_token_seq(3) strtk_parse_col_token_seq(4)
            strtk_parse_col_token_seq(5) strtk_parse_col_token_seq(6) strtk_parse_col_token_seq(7)
                strtk_parse_col_token_seq(8) strtk_parse_col_token_seq(9)
...
inline bool parse_columns(const InputIterator begin,
                          ...
                          Sequence<T, Allocator>& seq)
{
    strtk_parse_columns_impl(10) strtk_parse_col_token_seq(0) strtk_parse_col_token_seq(1)
        strtk_parse_col_token_seq(2) strtk_parse_col_token_seq(3) strtk_parse_col_token_seq(4)
            strtk_parse_col_token_seq(5) strtk_parse_col_token_seq(6) strtk_parse_col_token_seq(7)
...
inline bool parse_columns(const InputIterator begin,
                          ...
                          Sequence<T, Allocator>& seq)
{
    strtk_parse_columns_impl(9) strtk_parse_col_token_seq(0) strtk_parse_col_token_seq(1)
        strtk_parse_col_token_seq(2) strtk_parse_col_token_seq(3) strtk_parse_col_token_seq(4)
            strtk_parse_col_token_seq(5) strtk_parse_col_token_seq(6) strtk_parse_col_token_seq(7)
...
inline bool parse_columns(const InputIterator begin,
                          ...
                          Sequence<T, Allocator>& seq)
{
    strtk_parse_columns_impl(8) strtk_parse_col_token_seq(0) strtk_parse_col_token_seq(1)
        strtk_parse_col_token_seq(2) strtk_parse_col_token_seq(3) strtk_parse_col_token_seq(4)
...
inline bool parse_columns(const InputIterator begin,
                          ...
                          Sequence<T, Allocator>& seq)
{
    strtk_parse_columns_impl(7) strtk_parse_col_token_seq(0) strtk_parse_col_token_seq(1)
        strtk_parse_col_token_seq(2) strtk_parse_col_token_seq(3) strtk_parse_col_token_seq(4)
            strtk_parse_col_token_seq(5) strtk_parse_col_token_seq(6) return true;
}
...
inline bool parse_columns(const InputIterator begin,
                          ...
                          Sequence<T, Allocator>& seq)
{
    strtk_parse_columns_impl(6) strtk_parse_col_token_seq(0) strtk_parse_col_token_seq(1)
        strtk_parse_col_token_seq(2) strtk_parse_col_token_seq(3) strtk_parse_col_token_seq(4)
...
inline bool parse_columns(const InputIterator begin,
                          ...
                          Sequence<T, Allocator>& seq)
{
    strtk_parse_columns_impl(5) strtk_parse_col_token_seq(0) strtk_parse_col_token_seq(1)
...
inline bool parse_columns(const InputIterator begin,
                          ...
                          Sequence<T, Allocator>& seq)
{
    strtk_parse_columns_impl(4) strtk_parse_col_token_seq(0) strtk_parse_col_token_seq(1)
...
inline bool parse_columns(const InputIterator begin,
                          ...
                          Sequence<T, Allocator>& seq)
{
    strtk_parse_columns_impl(3) strtk_parse_col_token_seq(0) strtk_parse_col_token_seq(1)
...
inline bool parse_columns(const std::string& data,
                          ...
                          T11& t11)
{
    strtk_parse_col_begin() t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11 strtk_parse_col_end()
}
...
inline bool parse_columns(const std::string& data,
                          ...
                          T10& t10)
{
    strtk_parse_col_begin() t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10 strtk_parse_col_end()
}
...
inline bool parse_columns(const std::string& data,
                          ...
                          T9& t9)
{
    strtk_parse_col_begin() t0, t1, t2, t3, t4, t5, t6, t7, t8, t9 strtk_parse_col_end()
}
...
inline bool parse_columns(const std::string& data,
                          ...
                          T8& t8)
{
    strtk_parse_col_begin() t0, t1, t2, t3, t4, t5, t6, t7, t8 strtk_parse_col_end()
}
...
namespace details {
    inline bool
    type_to_string_converter_impl(const strtk::util::value& v, std::string& result, value_type_tag)
    {
...
namespace keyvalue {
    ...
    public:
        ...
        inline bool operator()(const range_type& data, const bool ignore_failures = false)
        {
            ...
            {
                pair_list_.clear();
                const std::size_t pair_count = split(pair_block_sdp_, data.first, data.second,
    ...
    namespace details {
        ...
        struct keygen<Range, unsigned int>
        {
            static inline unsigned int transform(const Range& key_range)
            {
                ...
                if(strtk::fast::numeric_convert(std::distance(key_range.first, key_range.second),
...
strtk_register_pair_to_ostream(char*) strtk_register_pair_to_ostream(unsigned char*)
    strtk_register_pair_to_ostream(const char*) strtk_register_pair_to_ostream(const unsigned char*)
...
namespace information {
    ...
    static inline std::string data()
    {
        static const std::string info_str = std::string(library) + std::string(" v")
            + std::string(version) + std::string(" (") + date + std::string(")");
        return info_str;
    }
